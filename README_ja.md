...
- 配置先に同じプレフィックスの `<入力 MD basename>-<docHash>-feedback.json` がすでに存在する場合は自動で読み込み、生成 HTML にコメントを inline する（前ラウンドのコメントを引き継いで続きから再開できる）。docHash 不一致時は stderr 警告を出して skip。stdin 入力時は cwd 誤読を避けるため skip
...
```bash
vp build        # dist/standalone.html / dist/embed-template.html / dist/review-request.mjs を生成
