...
npx mdxg-redline --sidebar-width 480 <input.md>   # コメントサイドバーの初期幅を 480px に指定
npx mdxg-redline --sidebar-width 0 <input.md>     # コメントサイドバーを閉じた状態で起動 (画面右端の縦タブのみ)
...
- `--sidebar-width <0|240-640>` で生成 HTML の `<html data-sidebar-width>` 属性を上書き。`0` は起動時 closed（画面右端の縦タブのみ表示）、`240–640` は open かつその幅 (px 整数)。`--theme` と同じく **ユーザーが UI でリサイズ・開閉した履歴 (`localStorage`) を最優先**し、次にこのヒント、最後に既定値 (`360px` / open) の順で決定する（未指定時は属性を付けず既存挙動を維持）
