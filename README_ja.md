...
npx mdxg-redline --comments-width 0 path/to/draft.md     # コメント機能を非表示にして単なるマークダウンビューワーとして開く
```

#### LLM が skill 経由で CLI を起動する場合

```bash
# gh skill install でのskillインストール例
gh skill install oubakiou/mdxg-redline md-review --agent claude-code --scope project

# npx skills add でのskillインストール例
...
| `--lang <auto\|en\|ja>`                  | CLI の help / エラーメッセージの出力言語。`auto` は env `$LC_ALL` → `$LC_MESSAGES` → `$LANG` から推定。**CLI 出力にのみ作用し、生成 HTML の表示言語には影響しない** (HTML 側はブラウザの `localStorage` / `navigator.language` から独立に決定する) | `auto`                  |
| `--help`                                 | 使い方ヘルプを表示して終了                                                                                                                                                                                                                         | —                       |
...
スマートフォン (iOS Safari / Android Chrome、幅 ≤ 768px) では、3 列レイアウトの代わりに上部ヘッダ + 画面下端のフッターバー + 左右 drawer モデルに切り替わる。`Open ▾` / `⚙ Settings` はヘッダに残り、`TOC` / `Comment` / `Search` の 3 ボタンは画面下端のフッターから操作する。TOC / Comment ボタンは左右の drawer を開閉し、Search ボタンは検索バーを開く。drawer は背景の暗幕タップ・`Esc`・同じフッターボタンの再押下のいずれでも閉じる。Help (キーボードショートカット一覧) はタッチ操作からは開けず、Bluetooth キーボード接続時の `h` キーからのみ利用できる（mobile では `#btn-help` を非表示にするため）。
