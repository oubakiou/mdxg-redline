...
| [追加] mobile で上下フリック時に virtual-page 境界へ吸着する (ページめくり感の第一弾)                                                                                                | 第一弾 | `.doc-pane { scroll-snap-type: y proximity; scroll-padding-top: 5% }` + `.virtual-page { scroll-snap-align: start }` で境界吸着。 連続通読を壊さないため `proximity` を選択。 「1 フリック=1 ページ」要望には未達 (proximity は位置ベース吸着で 1 フリック=1 ページを保証しない)、 厳密達成は後続でページャ化を検討（§5.u）                                                                                                                                                                                                                                                                                                                                                                                                  |
...
### u. mobile での virtual-page フリック移動 — 第一弾は CSS scroll-snap による境界吸着

「スマホで上下フリックで 1 ページ分移動したい (ページめくり感)」要望への **第一弾**。`@media (max-width: 768px)` で `.doc-pane { scroll-snap-type: y proximity; scroll-padding-top: 5% }` + `.virtual-page { scroll-snap-align: start }` を追加し、 通常スクロールはそのままに、 指を離した位置がページ境界近傍のときページ先頭へ吸着させる。

| 候補                                              | 採用         | 理由                                                                                                                                                  |
| ------------------------------------------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. `scroll-snap-type: y proximity` (境界吸着)** | ✓ (第一弾)   | 画面より縦に長い virtual-page も途中で止めて読める。 ジェスチャがスクロールと競合しない・キーボード / AT のスクロールを阻害しない・CSS のみで実装最小 |
| B. `scroll-snap-type: y mandatory` (強制吸着)     | ✗            | ページ途中で止まれず長文が読めない。 キーボードで 1 行ずつスクロールできなくなる (a11y 退化)                                                          |
| C. 各 virtual-page を `100dvh` 固定のカルーセル化 | ✗ (将来候補) | 厳密な「1 フリック=1 ページ」を満たすが、 「紙シートを連続スクロールで通読」という製品コンセプト (Stacked View) を捨てることになる。 別タスクで検討   |

採用案 A の論点と **要望への達成度ギャップ**：

- **proximity を選んだ核心**：本ツールの Virtual Pages は「H1 / H2 で区切った紙シートを縦に積み、 スクロールだけで全文通読する (Word / Pages 風)」のが製品コンセプト (README / DESIGN.md §6-7)。 `mandatory` はこの連続通読を壊すため不可。 proximity は「境界付近で止めたときだけ吸着」なので連続通読と両立する
- **`scroll-padding-top: 5%` を `SECTION_TOP_RATIO` (0.05) と揃える**：プログラム移動 `alignSectionTopInPane` (`navigation-orchestrator.ts`) は section top を `pane.clientHeight * 0.05` 下げた位置へ scrollTo する。 snap の `scroll-padding-top: 5%` も同じ scrollport 基準なので、 フリック吸着と TOC / hash 由来のプログラム移動の着地点が一致する。 page-scroll-spy の rootMargin (`-5% 0px -95% 0px`) の判定線とも揃う。 **CSS の `5%` と TS の `SECTION_TOP_RATIO = 0.05` はビルド境界が異なり変数共有できないため二重管理になる**点は既知の rot リスク (片方変更時はもう片方も追従させる)
- **「1 フリック=1 ページ」要望に対しては未達 (第一弾の割り切り)**：proximity は **指を離した最終スクロール位置の近傍**にしか吸着しないため、 1 フリックで必ず次ページ先頭へ着地する保証はない。 さらに `scroll-snap-stop` 未指定 (+ proximity では `stop` の効きが弱い) のため、 勢いの強い慣性フリックで複数 virtual-page を貫通し得る。 厳密達成には案 C (`100dvh` カルーセル) または JS でのフリック検出 → `navigateToTarget(activePageIndex ± 1)` が必要。 第一弾は「連続通読を壊さず、 境界吸着でページ単位の停止感を出す」ところまでを射程とする
- **既存スクロール機能との相性 (要実機検証)**：検索ハイライト / コメントカード → 本文ジャンプは `scrollIntoView({ block: 'center' })` でターゲットを画面中央に出す。 snap は `align: start` なので思想が異なり、 ジャンプ直後に画面へ軽く触れて離すと近傍ページ先頭へ吸着して中央位置がずれる可能性がある。 通常は center 着地が snap point から遠く吸着は発火しにくいが、 iOS Safari の慣性スクロール中の snap 干渉と併せて実機確認する
- **motion 制御 (§5.i) との関係**：`scroll-behavior` は未指定なので snap 移動はブラウザ既定 (多くは instant 寄り) で、 reduced-motion 下でも大きな視差は出ない。 現状は §5.i の合成クエリ (`@media (max-width: 768px) and (prefers-reduced-motion: reduce)`) に scroll-snap 用の追記はしない判断
