...
| [追加] mobile で 1 画面分のページ送りができる                                                                                                                                        | 実装済み | 画面左下の専用 FAB (`#btn-page-scroll`) をタップ=下 1 画面 / 上下フリック=その方向に 1 画面送り。 読む面のネイティブスクロールと操作を分離して競合を避ける。 drawer / コメントバー表示中は hide、 breakpoint 切替で focus を退避（§5.u）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
### u. mobile での 1 画面送り — 画面左下の page-scroll FAB

「スマホで上下フリックで 1 画面分スクロールしたい (ページめくり感)」要望への対応。**先行で試した scroll-snap (proximity による virtual-page 境界吸着) は要望と別物だったため revert し**、 採用したのは **読む面 (doc-pane) のネイティブスクロールから操作を分離した専用 affordance** ＝ 画面左下に浮かせた FAB (`#btn-page-scroll`, `src/app/chrome/page-scroll-button.ts`)。

| 候補                                           | 採用 | 理由                                                                                                                        |
| ---------------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------- |
| A. 読む面の上下フリックを乗っ取って 1 画面送り | ✗    | フリックは「読むためのスクロール」と同一操作。 乗っ取るとネイティブスクロールと競合し、 速度/距離での判別はブラウザ差で脆い |
| **B. 専用 FAB (左下) をタップ / フリック**     | ✓    | ページ送りを読む面から切り離すので競合ゼロ。 可視ボタンで発見性が高く、 本物の `<button>` で a11y も担保できる              |
| C. 前/次ページボタンを footer に増設           | ✗    | footer は既に 3 ボタンで、 増設すると tap target が窮屈。 また「フリックで方向指定」という要望の操作感に合わない            |

採用案 B の論点：

- **操作モデル**：タップ=下に 1 画面送り (読む方向の主操作)。 上フリック=上、 下フリック=下に 1 画面。 `touchend` の縦移動量 (`endY - startY`) が `FLICK_THRESHOLD_PX` (12px) を超えたら flick と判定し、 `preventDefault` で後続の合成 `click` を抑止して二重送りを防ぐ。 閾値未満は tap として `click` 経路 (= 下送り) に落とすことで、 mouse / keyboard (Enter / Space) も同経路で動く
- **`touchmove` の preventDefault は閾値超過後のみ**：常時抑止すると微小ジッタを伴う tap の合成 `click` まで阻害し「タップしても送られない」端末が出る。 移動量が閾値を超えてから抑止し、 tap の click を温存する
- **1 画面送り量と overlap**：`screenStep(clientHeight) = round(clientHeight * (1 - 0.12))`。 直前画面の最下部を ~12% 残して文脈を繋ぐ (PageDown の慣行)。 reduced-motion 時は `behavior: 'auto'`、 通常は `'smooth'`
- **既存 chrome との共存 / z-index**：FAB は `z-index: 50` で、 backdrop (55) / drawer (60) / footer (70) / search-bar (80) / modal-backdrop (100) より下。 modal 類は暗幕 (z 100) が覆うので明示 hide 不要。 一方 **drawer 展開中 (`body.mobile-drawer-open`) と範囲選択のコメントバー表示中 (`:root.floater-active`) は同じ左下〜下端で重なるため CSS で明示 hide** する。 `floater-active` は `setFloaterVisible` (`floater.ts`) が floater の表示と同期して `<html>` に付け外しする (floater の display 変更経路を全てこの helper に集約)
- **breakpoint focus-escape (§5.j-3)**：FAB は focusable な mobile 専用 button なので、 focus が乗ったまま desktop 幅へ resize すると不可視要素に取り残される。 `willBeHiddenOnDesktopEntry` (`mobile-footer.ts`) の hide 対象に `.page-scroll-fab` を加え、 既存の `.doc-pane` 退避フローに乗せる
- **配置 (左下) / tap target**：ユーザー選択により左下。 右利き片手では親指リーチがやや遠いが許容範囲 (将来不満が出れば配置切替の余地)。 48×48 で WCAG 2.5.5 (最小 44px) を満たす。 `safe-area-left` を尊重し `bottom: calc(var(--mobile-footer-height) + 16px)` で footer の上に浮かせる
- **フリック可能であることの発見性 (2 つの手がかり)**：単独の下向きシェブロンだと上フリックで戻れることが見えないため、 性質の違う 2 つの cue を併用する。 (1) **出現時バウンス** (操作前の誘い)：wire 時に `.is-hinting` を一定時間付与し、 FAB を上→中央→下→中央へ往復 (`@keyframes page-scroll-fab-hint`, 振幅 7px / 1.4s / 2 回) させて「縦両方向に動く」ことを示す。 display トグルでの再生を避けるため localStorage を使わず「wire 時に付与し合計時間後に外す」一発方式。 これは「毎回ロードで再生」を意味するが、 振幅が控えめ (48px に対し ~15%) かつ自動停止 (WCAG 2.2.2 の 5s 未満) なので冗長すぎないと判断 (端末で一度きりにしたい場合は localStorage 追加の余地)。 **`onTouchStart` で `.is-hinting` を除去**し、 ユーザーが触れた時点で誘いを止めて (2) の確認モードへ切り替える (バウンスと指追従の二重モーションを避ける)。 (2) **指追従の矢印** (操作中の確認)：ドラッグ中アイコンを指へ追従 (`iconDragStyle`: `translate` を ±8px に clamp) させ、 上方向に閾値超過したら `rotate(180deg)` で上向きシェブロンに反転する (「離せば上に戻る」を正直に示す)。 これが「上フリックが見えない」課題に直接効く。 **`translate` / `rotate` を個別プロパティに分け、 CSS の `transition` は `translate` のみ対象にする**：同一 `transform` 文字列だと指を離した戻りで `rotate` も補間されてアイコンがクルッと回る (戻りスピン) ため、 向きは瞬時に切り替えて位置だけ滑らかに戻す。 `transform-origin: center` で中心回転を保証。 reduced-motion ではバウンス (装飾) を無効化し、 指追従 (直接操作) は残しつつ `translate` の戻り transition を即時化する (§5.i)
- **割り切り (既知の制約)**：(1) 上記 2 cue はいずれも「動かせそう」「触れて初めて分かる」型で、 **初見で "上フリックで戻れる" という方向の意味論まで伝えきるには弱い** (バウンスは対称で方向中立、 指追従は接触後にしか効かない)。 方向まで明示するなら初回のみの短いテキストヒント (coachmark) が最も直接的で、 将来の補完候補。 aria-label には操作モデルを明記済み。 (2) 1 画面に収まる短い文書ではタップが no-op になる (`scrollBy` がクランプ)。 スクロール不能時の hide は将来の改善余地
