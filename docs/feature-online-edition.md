...
| 要件                                                                                  | 現状            | 完了条件                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [MUST] `dist/online.html` を第 3 の配布物として追加                                   | ✓               | `vp build` で `dist/online.html` が出力され、ホスティング先で `/online.html` を開くとビューワーが起動する                                                                                                                 |
| [MUST] `?url=<https://...>` で markdown を fetch して描画                             | ✓               | `?url=https://raw.githubusercontent.com/.../README.md` で開くと該当 markdown が描画される                                                                                                                                 |
| [MUST] 画面上から URL を指定する UI を提供                                            | 未 (Step 5)     | toolbar 等から常時アクセス可能な「Open URL」UI で URL を入力すると、その URL がクエリに反映された状態で再描画される                                                                                                       |
...
| [MUST] fetch 失敗時の graceful fallback                                               | toast ✓ / UI 未 | 404 / CORS エラー / timeout / 非 text レスポンスのそれぞれで、画面に分かるエラー表示が出て空状態か再試行 UI に落ちる (Step 4 は toast、richer error modal は Step 5)                                                      |
...
2. URL クエリ ?url= の処理（オンライン版のみ）
   ...
   2d. url 指定ありで http(s) なら fetchMarkdownFromUrl(url, opts, allowlist) を呼ぶ
       ...
       - ok: text 取得 → loadFromMarkdown(deriveDocNameFromUrl(url), text)
                       → DESIGN.md §9 既存経路 1c (embedded-feedback 適用) 1d (render) に合流
       - エラー: §5.d のカテゴリ別 toast + 4 へ fallthrough
   ...
   2e. online edition では 2 の結果に関わらず 3 (embedded-md) を skip
       (online.html に placeholder embedded-md が事故注入された場合の
        「toast + 無関係 doc 描画」経路を構造的に塞ぐ)

3. embedded-md (<script id="embedded-md">) があれば適用（既存、online.html では到達しない）

4. それ以外は空状態のまま Open file 待ち（Step 5 で Open URL ボタンも追加）
```

**実装関数対応** (Step 4 で実装済み):

| 擬似コードステップ      | 実装関数 (`src/app/boot.ts`)                                                                               |
| ----------------------- | ---------------------------------------------------------------------------------------------------------- |
| 1a                      | `isOnlineEdition()`                                                                                        |
| 2a, 2b                  | `readUrlQuery()`                                                                                           |
| 2c                      | `isHttpProtocol(globalThis.location.protocol)` 判定                                                        |
| 2d (allowlist 読み込み) | `readOnlineAllowlistFromDom()` (`src/core/online-allowlist-config.ts:resolveOnlineAllowlistFromJson` 経由) |
| 2d (fetch + 合流)       | `tryFetchAndLoad()` → `fetchMarkdownFromUrl()` → `runtime.loadFromMarkdown()`                              |
| 2d (エラーメッセージ)   | `formatFetchFailureMessage(failure)` + `toast(...)`                                                        |
| 1a + 2 全体             | `loadFromOnlineUrlQuery(runtime)` (orchestrator)                                                           |
| 2e                      | `boot()` の `isOnlineEdition()` ガード後の `return`                                                        |

**順序の根拠**: `?url=` 未指定で `file://` から online.html を開いた場合、ユーザーは URL fetch を発火していないため、protocol 警告を出すのは過剰。`?url=` 指定時のみ protocol 警告を発火させ、それ以外は standalone と等価な空状態にすることで、§5.e の「`file://` 起動時にも Open file 経路は動かす」方針と整合する。Open URL ボタン経由の protocol 警告は Step 5 で modal の送信ハンドラ内で再チェックする (Step 4 時点では UI 未実装)。
...
- `location.protocol` 検証（`http:` / `https:` 以外はエラー toast）
...
- 各 fetch 失敗は `formatFetchFailureMessage(failure)` でカテゴリ別メッセージを生成し、当面 toast で表示。richer error modal は Step 5 で追加

成果物：

- `?url=<valid>` を allowlist 内 origin で開くと markdown が描画される
- `?url=` **未指定** → 無音 fallthrough で空状態 (toast なし、Open file 経路は残る)
- `?url=` **指定 + protocol 不一致 / 無効 URL / allowlist 外 / fetch 失敗** → §5.d カテゴリ別の **toast** + 空状態 (richer error modal は Step 5 で追加)
...
- 送信時に `?url=<入力 URL>` を `location.assign` で反映して同一ページを reload（URL を共有可能にするため）
...
- `app/boot.ts`（既存テストに追加、Step 4 で実装済み）:
  ...
  - 残り (Step 5 待ち): toolbar の Open URL ボタン表示 / Open URL modal の JS gating
...
- [ ] `?url=` に 404 URL を渡すとエラー画面 + 再試行 UI
- [ ] toolbar の Open URL ボタンから URL を送信すると `?url=...` がクエリに反映されて描画される
