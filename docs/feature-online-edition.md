...
| [MUST] `?url=<https://...>` で markdown を fetch して描画                             | 未   | `?url=https://raw.githubusercontent.com/.../README.md` で開くと該当 markdown が描画される                                                                                                                                 |
...
2. URL クエリ ?url= の処理（オンライン版のみ）
   ...
   2d. url 指定ありで http(s) なら allowlist 検証
       - allowlist hit: fetch → text 取得 → docHash 計算 → loadFromMarkdown(text, basename(url))
...
- [ ] `?url=` に allowlist 外ドメインを渡すとエラー画面 + allowlist 一覧
