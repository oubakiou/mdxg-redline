...
| 要件                                                                                  | 現状              | 完了条件                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [MUST] `dist/online.html` を第 3 の配布物として追加                                   | build ✓ / boot 未 | `vp build` で `dist/online.html` が出力され、ホスティング先で `/online.html` を開くとビューワーが起動する                                                                                                                 |
| [MUST] `?url=<https://...>` で markdown を fetch して描画                             | core ✓ / boot 未  | `?url=https://raw.githubusercontent.com/.../README.md` で開くと該当 markdown が描画される                                                                                                                                 |
...
| [MUST] オンライン版独自の CSP で URL fetch 対象を allowlist 化                        | build ✓           | `dist/online.html` の `<meta http-equiv="Content-Security-Policy">` が `connect-src` に明示 allowlist を持つ                                                                                                              |
| [MUST] fetch 失敗時の graceful fallback                                               | 未                | 404 / CORS エラー / timeout / 非 text レスポンスのそれぞれで、画面に分かるエラー表示が出て空状態か再試行 UI に落ちる                                                                                                      |
| [MUST] `file://` 起動時はオンライン版機能を発火させない                               | 未                | `?url=` 指定時のみ URL fetch を skip し、エラー画面 + "online edition は http(s) 必須" + Open file fallback。`?url=` 未指定時は standalone と等価な空状態で起動（§3.4 / §5.e）                                            |
...
2. URL クエリ ?url= の処理（オンライン版のみ）
   ...
   2d. url 指定ありで http(s) なら fetchMarkdownFromUrl(url, opts, allowlist) を呼ぶ
       ...
       - ok: text 取得 → docHash 計算 → loadFromMarkdown(text, basename(url))
                       → DESIGN.md §9 既存経路 1c (embedded-feedback 適用) 1d (render) に合流
       - エラー: §5.d のカテゴリ別エラー画面に分岐
...
**順序の根拠**: `?url=` 未指定で `file://` から online.html を開いた場合、ユーザーは URL fetch を発火していないため、protocol 警告のエラー画面に落とすのは過剰。`?url=` 指定時のみ protocol 警告を発火させ、それ以外は standalone と等価な空状態にすることで、§5.e の「`file://` 起動時にも Open file 経路は動かす」方針と整合する。Open URL ボタンは toolbar に表示されたままだが、クリック時の送信ハンドラ内で protocol を再チェックし、`file://` では「http(s) でホスティングされた online edition でのみ機能」の警告を出す。
...
成果物：online.html を `?url=<valid>` で開くと markdown が描画される。`?url=` 未指定 / 無効 URL / fetch 失敗時のフォールバック画面が出る。
...
- `app/boot.ts`（既存テストに追加）：
  ...
  - `?url=` なしのオンライン版で空状態 + toolbar の Open URL ボタンが表示される
  - `?url=` ありで fetch 経路が呼ばれる
  - allowlist 外 URL でエラー画面に分岐する
  - `file://` 起動 + `?url=` 未指定で空状態に到達（エラー画面に落ちない、§3.4 順序の根拠）
  - `file://` 起動 + `?url=` 指定でエラー画面 + Open file fallback
