...
| 要件                                                                        | 現状 | 完了条件                                                                                                                                                                                                                                                                                                 |
| --------------------------------------------------------------------------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [SHOULD] `$...$` で囲まれたインライン数式が周囲のテキストとインラインに描画 | 未   | `dist/standalone.html` は build 時に KaTeX を `<script id="embedded-katex">` / `<style id="embedded-katex-css">` に inline（§5.k）、CLI 経路（`embed-template.html` → `*-review.html`）は `--math <auto\|on\|off>` モードで注入判定（既定 `auto`）。`$...$` を `<span class="katex">` として inline 描画 |
...
| ファイル                   | 内容                                                                                                                                                                                                                               | 配布形態                             |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `dist/standalone.html`     | 単独 Open file 用、Shiki bundled 全言語（約 235）inline 済み + Mermaid inline 済み + **KaTeX 本体 / CSS / フォント (data URI) も build 時に inline 済み**（Shiki / Mermaid と同じ「default で全部入り」の配布契約、設計判断 §5.k） | エンドユーザーが直接ダブルクリック   |
| `dist/embed-template.html` | review-request CLI が rewrite テンプレートとして読み込む素材。KaTeX 本体 / CSS / フォントは **含まない**（CLI `--math` モードに応じて動的に注入する）                                                                              | CLI 経由でのみ使用、直接開く想定なし |
| `dist/katex/katex.mjs`     | KaTeX の ESM bundle（vite で別 entry として emit、commit 対象）。standalone build plugin と CLI の双方が読み込む素材                                                                                                               | CLI / standalone build の入力        |
| `dist/katex/katex.css`     | KaTeX の CSS（フォント `@font-face` は data URI に書き換え済み、commit 対象）。standalone build plugin と CLI の双方が読み込む素材                                                                                                 | CLI / standalone build の入力        |
| `dist/review-request.mjs`  | CLI 本体。`--math` モードに応じて `dist/katex/*` を読み、`embed-template.html` を rewrite した `*-review.html` の `<script id="embedded-katex">` と `<style id="embedded-katex-css">` に書き込む                                   | 配布者向け CLI                       |

CLI が生成する配布 HTML（`<mdFileName>-<docHash>-review.html`）のサイズは KaTeX 非注入時（`--math off` または `auto` で 0 件）には現行の `embed-template.html` ベース（~327 KB raw / ~99 KB gzipped、DESIGN.md §12 §2 Code Block Rendering 行）から変動しない。`--math on` 時 / `auto` 検出時のみ +120〜180 KB gzipped 程度の増加（KaTeX JS + CSS + フォント data URI 化分の和）。この設計により、数式を必要としないレビューフロー（仕様書 / 散文中心の文書）が肥大化を被らない。

`dist/standalone.html` には KaTeX runtime を build 時に inline する（設計判断 §5.k）。Shiki bundled 全言語 / Mermaid と同じ「default で全部入り」配布契約を維持し、standalone を直接ダブルクリックで開いたユーザーも `$...$` / `$$...$$` を KaTeX 描画で受け取れる。サイズ増分は KaTeX JS + CSS + フォント data URI 込みで gzip +135 KB 程度（現行 ~45 MB / gzip ~5.9 MB → ~46 MB / gzip ~6.7 MB 目安）で許容範囲。
...
`src/cli/parse-args.ts` に `--math <auto|on|off>` mode を追加する（`--shiki-langs` / `--mermaid` と同じ mode 型インターフェース）。既定は `auto`。
...
CLI 生成 `*-review.html`（`embed-template.html` ベース）と `dist/standalone.html` への KaTeX 注入後のサイズ見積もり：

| ケース                                                                | 配布 HTML raw                         | KaTeX JS 増分                | KaTeX CSS + フォント増分                            | gzipped 後の配布物    |
| --------------------------------------------------------------------- | ------------------------------------- | ---------------------------- | --------------------------------------------------- | --------------------- |
| `*-review.html`: `--math off` または `auto` で 0 件（後者が既定挙動） | ~327 KB（現行 `embed-template.html`） | 0 KB                         | 0 KB                                                | ~99 KB（現行）        |
| `*-review.html`: `--math on` または `auto` で 1 件以上                | ~327 KB                               | +280 KB raw / +75 KB gzipped | +700 KB raw（フォント data URI 含）/ +60 KB gzipped | ~234 KB               |
| `dist/standalone.html`: KaTeX 同梱（build 時 default、§5.k）          | ~45 MB（現行）                        | +280 KB raw / +75 KB gzipped | +700 KB raw / +60 KB gzipped                        | ~6.7 MB（~46 MB raw） |

実測値は Step 1 PoC 完了後に確定する。CLI 経路の `auto` 検出 / `on` 注入時に stderr へ「Detected N math expression(s). Embedding KaTeX runtime (+135 KB gzipped).」を出す。
...
- **standalone への inline**: `vite.config.ts` の `mdxg-split-outputs` plugin（あるいは新規 `mdxg-katex-asset` plugin）が closeBundle で `dist/katex/katex.mjs` / `katex.css` を読み、`</script>` sanity check を施した上で `dist/standalone.html` の `<script id="embedded-katex" type="module">` と `<style id="embedded-katex-css">` の 2 ブロックに inline する。CLI 経由（embed-template.html → \*-review.html）と standalone build で同じ bridge 注入規約（`globalThis.__mdxgKatex` セット + `mdxg:katex-ready` イベント）を共有するため、ブラウザ側 `src/app/katex.ts` のロジック分岐は不要

成果物：`dist/katex/katex.mjs` と `dist/katex/katex.css` が emit され commit 対象になる + `dist/standalone.html` に KaTeX runtime / CSS / フォント data URI が inline 済み

### Step 4: CLI 側 — `--math <auto|on|off>` mode と注入

- `src/cli/parse-args.ts` に `--math <auto|on|off>` mode を追加（既定 `auto`）。`parseMathValue` は `parseMermaidValue` / `parseShikiLangsValue` と同じ pattern で実装し、未知の値はエラーで reject
- `src/cli/review-request.ts` のエントリで mode と `countMath(markdown).inline + countMath(markdown).display` から「注入する / しない」を解決：
  - `auto`: 件数 > 0 なら注入
  - `on`: 常に注入
  - `off`: 常に注入しない
- 注入する場合は `dist/katex/katex.mjs` と `dist/katex/katex.css` を読み込み（`readFileSync`）、`core/embed.ts` の `katexRuntime` 引数（`{ js, css }`）に渡す
...
- HELP_TEXT 更新：`--math <auto|on|off>` の意味（auto = 自動検出、on = 常時注入、off = 注入しない。注入時の配布物サイズが +135 KB gzipped 程度増える）と既定値（auto）を記述
- 注入判定時に CLI が stderr へ「Detected N math expression(s). Embedding KaTeX runtime (+135 KB gzipped).」と件数だけ報告し、配布者が気づける導線を残す（Mermaid と対称）

成果物：CLI が mode と検出結果から `embedded-katex` / `embedded-katex-css` 注入を決定できること（in-source test で auto / on / off × 0件 / 1件 の組み合わせを検証）
...
- DESIGN.md §3 review-request CLI コマンド仕様に `--math <auto|on|off>` を追記（既定 `auto`、auto の判定挙動とサイズ影響）
...
| `on`（常時 ON）      | ✗    | 数式を含まない markdown でも +135 KB gzipped 増える経路を既定にすると、他のレビューフロー（仕様書 / 散文中心）への影響が大きい                                                                                                                                                                                                                   |
...
- markdown を編集している途中で意図せず `$` が混入した場合、配布物サイズが突然 +135 KB 増える経路ができる。CLI が auto モードで注入判定時に stderr へ「Detected N math expression(s). Embedding KaTeX runtime (+135 KB gzipped).」と件数だけ報告し、配布者が気づける導線を残す
...
### k. `dist/standalone.html` に KaTeX を default で同梱する

| 候補                                                                              | 採用 | 理由                                                                                                                                                                                                                                                                                                                                                  |
| --------------------------------------------------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. `dist/standalone.html` に Shiki / Mermaid と同じく KaTeX を default inline** | ✓    | Shiki bundled 全言語化と Mermaid 同梱と同じ「default で全部入り」配布契約を維持。standalone を直接ダブルクリックで開いたユーザーも `$...$` / `$$...$$` を KaTeX 描画で受け取れる。サイズ増（KaTeX JS + CSS + フォント data URI 込みで gzip +135 KB、~45 MB → ~46 MB、gzip ~5.9 MB → ~6.7 MB）は許容範囲。CLI 経路と挙動が分岐しないため UX が一貫する |
...
- CLI 経路（`embed-template.html` → `*-review.html`）の `--math <auto|on|off>` は維持する（§5.d / §5.e）。`embed-template.html` には KaTeX を default inline せず、CLI が markdown 内容に応じて注入する設計のままで、配布物サイズの最小化導線を残す。「standalone は full feature default / CLI 経路は配布物サイズ最適化のため opt-in」という役割分担で整合する（Mermaid §5.l と完全に対称）
- standalone build と CLI 経路の bridge 注入規約（`<script id="embedded-katex">` + `<style id="embedded-katex-css">` + `globalThis.__mdxgKatex`）は同一のため、ブラウザ側 `src/app/katex.ts` のロジック分岐は不要
- standalone build への inline は `mdxg-shiki-assets` plugin の closeBundle に倣って `mdxg-katex-asset` plugin（または既存 `mdxg-split-outputs` への統合）で `<script id="embedded-katex">` / `<style id="embedded-katex-css">` を埋め込む
...
- [ ] `--math auto` で math 含み markdown を渡した時、stderr に「Detected N math expression(s). Embedding KaTeX runtime (+135 KB gzipped).」が出る
- [ ] `--math on` で実行した HTML のサイズが見積もり通り（~234 KB gzipped）
...
- [ ] `dist/standalone.html` のサイズが見積もり通り（~46 MB raw / ~6.7 MB gzipped）
...
- `*-review.html` の `--math on` / `auto` で 1 件以上のときのサイズ増分が **gzip +150 KB 以内**
- `dist/standalone.html` のサイズが KaTeX inline 後 **gzip +150 KB 以内**（現行 ~5.9 MB → ~6.7 MB gzip 目安、§3.1 / §5.k）
