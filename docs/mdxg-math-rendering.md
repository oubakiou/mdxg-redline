...
DESIGN.md §13 で `dist/` の出口は `dist/standalone.html` / `dist/embed-template.html` / `dist/review-request.mjs` / `dist/shiki-langs/*.json` の 4 系統に分かれる（旧 `dist/review.html` は split-outputs 化以降は build 中の中間出力としてのみ存在し、最終配布物には残らない）。KaTeX 対応の配布物は次の構成になる：

| ファイル                   | 内容                                                                                                                                                                                                    | 配布形態                             |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `dist/standalone.html`     | 単独 Open file 用、Shiki bundled 全言語（約 235）inline 済み。KaTeX 本体 / CSS / フォントは **含まない**（CLI 経路ではないため `--math` 注入対象外）。`$...$` / `$$...$$` は plain text fallback で表示 | エンドユーザーが直接ダブルクリック   |
| `dist/embed-template.html` | review-request CLI が rewrite テンプレートとして読み込む素材。KaTeX 本体 / CSS / フォントは **含まない**（`--math` 未指定時のデフォルト配布物）                                                         | CLI 経由でのみ使用、直接開く想定なし |
...
| `dist/review-request.mjs`  | CLI 本体。`--math` 指定時のみ `dist/katex/*` を読み、`embed-template.html` を rewrite した `*-review.html` の `<script id="embedded-katex">` と `<style id="embedded-katex-css">` に書き込む            | 配布者向け CLI                       |

CLI が生成する配布 HTML（`<mdFileName>-<docHash>-review.html`）のサイズは `--math` 未指定時（既定）には現行の `embed-template.html` ベース（~327 KB raw / ~99 KB gzipped、DESIGN.md §12 §2 Code Block Rendering 行）から変動しない。`--math` 指定時のみ +120〜180 KB gzipped 程度の増加（KaTeX JS + CSS + フォント data URI 化分の和）。この設計により、数式を必要としないレビューフロー（仕様書 / 散文中心の文書）が肥大化を被らない。

**`dist/standalone.html` への KaTeX 同梱は本タスクでは行わない**（設計判断 §5.k 相当、`docs/mdxg-diagram-rendering.md` §5.l と同じ思想）。standalone を直接ダブルクリックするユーザーは `$...$` / `$$...$$` を plain text として読む（MDXG §14 [MUST] 生の文法保持は満たす）。理由：standalone は既に Shiki bundled 全言語 inline で ~45 MB / gzip ~5.9 MB と肥大化しており、KaTeX を追加で載せると build / 起動コストが上乗せされる。standalone から数式描画を望むユースケースが顕在化した時点で再評価する。
...
CLI が生成する `*-review.html`（`embed-template.html` ベース、~327 KB raw / ~99 KB gzipped）に対する KaTeX 注入後のサイズ見積もり：

| ケース                | 配布 HTML raw                         | KaTeX JS 増分                | KaTeX CSS + フォント増分                            | gzipped 後の配布物 |
| --------------------- | ------------------------------------- | ---------------------------- | --------------------------------------------------- | ------------------ |
| `--math` なし（既定） | ~327 KB（現行 `embed-template.html`） | 0 KB                         | 0 KB                                                | ~99 KB（現行）     |
| `--math` あり         | ~327 KB                               | +280 KB raw / +75 KB gzipped | +700 KB raw（フォント data URI 含）/ +60 KB gzipped | ~234 KB            |

実測値は Step 1 PoC 完了後に確定する。`--math` 指定時に CLI が stderr へ「Math support enabled. Output HTML is ~234 KB gzipped (vs ~99 KB without --math).」を出す予定。

`dist/standalone.html` は §3.1 の通り KaTeX 注入対象外のため本表の影響を受けず、~45 MB raw / ~5.9 MB gzipped（現行）のまま変動しない。
...
### k. `dist/standalone.html` への KaTeX 同梱を見送る

| 候補                                                                  | 採用 | 理由                                                                                                                                                                                                                                                                                                                                |
| --------------------------------------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. CLI 経路（`*-review.html`）のみに `--math` で opt-in 注入**      | ✓    | CLI を通すユースケースは「特定 MD のレビュー固定文脈」で配布者が markdown 内容を制御している（DESIGN.md §3 「--show-open-file」の責務境界と同じ思想）。KaTeX runtime + フォントのサイズ影響（+120〜180 KB gzipped）を「実際に数式を含む配布」だけに局所化できる。standalone は MDXG §14 [MUST] 生の文法保持（plain text）で要件充足 |
| B. `dist/standalone.html` にも Shiki と同じく KaTeX を default inline | ✗    | standalone は既に Shiki bundled 全言語 inline で ~45 MB raw / ~5.9 MB gzipped と肥大化している。フォント data URI 化分も含めると +1 MB raw 程度の上乗せになり、`file://` ダブルクリック起動時のパース時間が劣化する。standalone を直接開くユースケースで数式描画が強く必要、というユーザー要望はまだ顕在化していない                |
...
- standalone を直接ダブルクリックで開いたユーザーは `$...$` / `$$...$$` を raw な markdown 文法のまま読む。視覚的に数式化されないことに気づける導線として inline hint を出す案もあるが、本タスクでは見送り（CLI 経由フローのドキュメント追記で代替）
- 将来 standalone への KaTeX 同梱が必要になった時の追加経路は、`mdxg-shiki-assets` plugin の closeBundle に倣って `mdxg-katex-asset` plugin を追加し `<script id="embedded-katex">` / `<style id="embedded-katex-css">` を standalone build 時に inline する形で開ける（本タスクの bridge 注入規約と同一なので、ブラウザ側ロジックは追加不要）。`docs/mdxg-diagram-rendering.md` §5.l の Mermaid 見送り判断と完全に対称
...
- [ ] CLI が生成する `*-review.html`（`--math` なし）のサイズが既存 `embed-template.html` ベースとほぼ同じ（~99 KB gzipped）
...
- CLI が生成する `*-review.html`（`embed-template.html` ベース）のサイズが `--math` 未指定時に **既存と変動なし**。`dist/standalone.html` のサイズも本タスクでは変動させない（§3.1 設計判断）
