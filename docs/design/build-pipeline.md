...
> 本書は [DESIGN.md](./DESIGN.md) の旧 §13「ビルドパイプライン」を独立ドキュメントとして切り出したもの。参照互換のため見出し番号 §13 を維持する。記述は DESIGN.md と同じ[編集規約](./DESIGN.md#12-mdxg-準拠状況と設計判断)（設計レイヤーの WHY に絞り、実装スナップショット = file:symbol / 行数 / 実測サイズ はコードと archive へ委譲）に準拠する。**`mdxg-split-outputs` plugin の処理は本節「全体像」の 10 ステップを正典**とし、責務分担表はそれを要約する。
...
7. `dist/standalone.html` から `buildOnlineHtml(standaloneHtml, { allowlist, manifest })` で `dist/hosting/index.html` を派生 (Pages 慣習に合わせて `online.html` → `index.html` リネーム、archive 化された `docs/archive/feature-online-runtime-assets.archive.md` Step 4 の C 設計判断)。次の 9 mutation を apply: (a) `<html data-mdxg-online="1">` upsert、(b) CSP `connect-src 'none'` → `connect-src 'self' <allowlist origins>` 置換 (`'self'` は同一オリジン同梱資材 fetch / dynamic import 用、DESIGN.md §11.b)、(c) `<script id="online-allowlist">` JSON 注入、(d) `<script id="online-asset-manifest">` JSON 注入 (manifest 経由で loader が fingerprinted パスを解決、archive doc §3.2)、(e) `<script id="embedded-shiki-langs">` の textContent を空 `{}` に上書き、(f) `<script id="embedded-mermaid">` の textContent を空に上書き、(g) `<script id="embedded-katex">` の textContent を空に上書き、(h) `<style id="embedded-katex-css">` / `<style id="embedded-katex-fonts-extra-css">` の textContent を空に上書き (素材契約として block 不在は fail-fast、Shiki と対称)、(i) `<head>` に OGP / Twitter Card の固定 `<meta>` を inject (`og:url` / `og:image` は `https://mkdn.review` 絶対 URL。crawler は JS 非実行で `?url=` 先を読めないため全 URL 共通の固定カードとする。素材 standalone / embed-template には載せず online 派生でのみ付与)。これにより `dist/hosting/index.html` が「最小 shell」となり、必要なアセットはすべて runtime fetch / dynamic import で動的取得される。allowlist は `buildOnlineAllowlist(process.env)` で env var `MDXG_ONLINE_CONNECT_SRC` を読んで正規化、解決済み allowlist を stdout に emit して build 再現性を保証する
...
9. `emitHosting404()` / `emitHostingOgImage()` で `dist/hosting/` 配下の静的ファイル 2 種を emit: `404.html` は Cloudflare Pages の SPA fallback (未知パス → index.html を 200 返し) を抑制する空 shell、`og-image.png` は OGP カード画像 (1200x630 PNG)。後者は `assets/og-image.png` をコピーするだけで、配信ビルドは rasterizer に非依存（`assets/og-image.png` は `assets/og-image.svg` から事前 rasterize した生成物。再生成手順は `assets/README.md`）
10. 中間出力 `dist/review.html` を `dist/embed-template.html` にリネーム（こちらは grammar / Mermaid / KaTeX いずれも注入なし、CLI が `--shiki-langs` / `--mermaid` / `--math` / `--math-fonts` モードに応じて動的に注入する）
...
各 config の役割を要約する。`mdxg-split-outputs` plugin の処理内容は上の「全体像」の 10 ステップを正典とし、ここでは入口を示すに留める。
...
| 出力分岐                    | mdxg-split-outputs     | closeBundle 内で逐次 await: grammar JSON を 3 系統 emit → Mermaid / KaTeX runtime を hosting に hash 焼き込み 2 系統 emit → standalone へ各 embed inline → standalone から online (`dist/hosting/index.html`) を 9 mutation で派生 → `_headers` / `404.html` / `og-image.png` emit → 中間出力を `dist/embed-template.html` にリネーム。各ステップの詳細は「全体像」の 10 ステップを参照 |
