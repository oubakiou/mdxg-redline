...
- **キーボードナビゲーションでの jump / back-jump**：footnote-ref を Enter で発火させると `#fn-<id>` に scroll + focus、backref を Enter で `#fnref-<id>` に戻る。`navigateToTarget` の hash 解決経路に脚注 hash パターン (`fn-<id>` / `fnref-<id>`) を追加（DESIGN.md §12 §13 Keyboard Navigation との整合）
...
1. `a[href^="#fn-"]` / `a[href^="#fnref-"]` を click delegate で捕捉
...
- `app/review.ts` の click delegate で `a[href^="#fn-"]` / `a[href^="#fnref-"]` を捕捉する footnote 専用 hash handler を追加：

  ```ts
  function handleFootnoteHashClick(hash: string): boolean {
    if (!/^#fn(ref)?-/.test(hash)) return false
...
- [docs/mdxg-math-rendering.archive.md](./mdxg-math-rendering.archive.md) — Math 設計プラン（`<sup class="footnote-ref">` の textSegments 経路の先行パターン参考元）
