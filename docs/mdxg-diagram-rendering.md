...
エンコード規約は `embedded-md` / `embedded-shiki-langs` と同じく `</script>` の誤検出を避けるため、ESM ソース中に `</script>` 文字列が含まれていないことをビルド時に sanity check し、見つかれば build を fail させる（実用上 Mermaid 本体には含まれない見込みだが、version up で混入する経路を構造的に塞ぐ）。
