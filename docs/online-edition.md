...
| `followRedirect`       | `manual` 相当（最大 3 hop まで本実装側で追跡）                                             | redirect chain の途中で CORS NG になるケースを早期検出し、リダイレクト loop を構造的に塞ぐ                                                                                                                                   |

これらは Step 1 PoC で実測してから固定する。`acceptedContentTypes` allowlist は wildcard を使わず明示列挙にする（攻撃面の縮小）。

成果物：`src/core/online-url.ts` + in-source test（正常系 / allowlist hit/miss / scheme チェック / 各 fetch 失敗ケース / 空 URL / timeout / maxBodyBytes 超過 / 非対応 content-type / redirect hop 超過）
