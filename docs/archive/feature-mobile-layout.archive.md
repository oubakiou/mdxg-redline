...
- **フリック方向の確認 (指追従の矢印)**：単独の下向きシェブロンだと上フリックで戻れることが見えないため、 ドラッグ中アイコンを指へ追従 (`iconDragStyle`: `translate` を ±8px に clamp) させ、 上方向に閾値超過したら `rotate(180deg)` で上向きシェブロンに反転する (「離せば上に戻る」を正直に示す)。 これが「上フリックが見えない」課題に効く。 **`translate` / `rotate` を個別プロパティに分け、 CSS の `transition` は `translate` のみ対象にする**：同一 `transform` 文字列だと指を離した戻りで `rotate` も補間されてアイコンがクルッと回る (戻りスピン) ため、 向きは瞬時に切り替えて位置だけ滑らかに戻す。 `transform-origin: center` で中心回転を保証。 reduced-motion では指追従 (直接操作) は残しつつ `translate` の戻り transition を即時化する (§5.i)
- **タップのフィードバック (矢印ナッジ)**：タップ (= 下 1 画面送り) で矢印を下へ ~6px 突いて戻し、 送り方向を確認させる (`playTapNudge`)。 指追従の inline `translate` / `rotate` とは別系統の一発再生にするため Web Animations API (`Element.animate`) を使う：CSS class の付け外し + reflow による再生し直しが不要で、 連打でも素直に再生される。 `onClick` 経路 (tap / keyboard) のみで発火する (flick の `touchend` は `preventDefault` で click を抑止するため二重再生しない)。 reduced-motion 時は再生しない
- **割り切り (既知の制約)**：(1) 指追従は「触れて初めて分かる」型なので、 **初見で "上フリックで戻れる" という方向の意味論まで伝えるには弱い**。 方向まで明示するなら初回のみの短いテキストヒント (coachmark) が最も直接的で、 将来の補完候補。 aria-label には操作モデルを明記済み。 (2) 1 画面に収まる短い文書ではタップが no-op になる (`scrollBy` がクランプ)。 スクロール不能時の hide は将来の改善余地
