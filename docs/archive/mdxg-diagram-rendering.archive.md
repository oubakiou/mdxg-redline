...
| 要件                                                                                    | 現状 | 完了条件                                                                                                                                                                                                                                                                                |
| --------------------------------------------------------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [SHOULD] 最低限、` ```mermaid ` ブロックはサポートされる                                | 未   | `dist/standalone.html` は build 時に Mermaid.js を `<script id="embedded-mermaid">` に inline（§5.l）、CLI 経路（`embed-template.html` → `*-review.html`）は `--mermaid <auto\|on\|off>` モードで注入判定（既定 `auto`）。配布 HTML 起動時に各 ` ```mermaid ` ブロックを SVG に upgrade |
...
| `core/markdown.ts` renderer の言語別分岐             | `mermaid` 識別子は Shiki ハイライト経路を **skip しない**（フォールバック確保のため Shiki ハイライトに乗せる）。識別用に `<pre data-mermaid="1">` 属性を付与し、後段の Mermaid upgrade 対象を絞る |
| Shiki upgrade フェーズ                               | `src/app/mermaid.ts`（新規）の lazy upgrade フェーズで `requestIdleCallback` 後に Mermaid SVG を生成                                                                                              |

本実装は Mermaid.js のサイズ（gzip ~700 KB）が単一 HTML 配布物に与える影響を踏まえ、経路ごとに異なる方針を採る：`dist/standalone.html` は Shiki bundled 全言語と同じく「default で全部入り」配布契約を維持し Mermaid を build 時に inline する（§5.l）。CLI 経路（`embed-template.html` → `*-review.html`）は `--mermaid <auto|on|off>` モード（既定 `auto`）で配布物サイズを最適化する opt-in 設計とし、markdown が `mermaid` ブロックを含むときだけ自動注入する。

## 3. bundle 構成と Mermaid 注入

### 3.1 配布物の構成

DESIGN.md §13 で `dist/` の出口は `dist/standalone.html` / `dist/embed-template.html` / `dist/review-request.mjs` / `dist/shiki-langs/*.json` の 4 系統に分かれる。Mermaid 対応の配布物は次の構成になる：

| ファイル                   | 内容                                                                                                                                                                   | 配布形態                             |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| `dist/standalone.html`     | 単独 Open file 用、Shiki bundled 全言語（約 235）inline 済み + **Mermaid.js も build 時に inline 済み**（Shiki と同じ「default で全部入り」の配布契約、設計判断 §5.l） | エンドユーザーが直接ダブルクリック   |
| `dist/embed-template.html` | review-request CLI が rewrite テンプレートとして読み込む素材。Mermaid.js は **含まない**（CLI `--mermaid` モードに応じて動的に注入する）                               | CLI 経由でのみ使用、直接開く想定なし |
| `dist/mermaid.mjs`         | Mermaid.js の ESM bundle（vite で別 entry として emit、commit 対象）。standalone build plugin と CLI の双方が読み込む素材                                              | CLI / standalone build の入力        |
| `dist/review-request.mjs`  | CLI 本体。`--mermaid` モードに応じて `dist/mermaid.mjs` を読み、`embed-template.html` を rewrite した `*-review.html` の `<script id="embedded-mermaid">` に書き込む   | 配布者向け CLI                       |

CLI が生成する配布 HTML（`<mdFileName>-<docHash>-review.html`）のサイズは Mermaid 非注入時（`--mermaid off` または `auto` で 0 件）には現行の `embed-template.html` ベース（~327 KB raw / ~99 KB gzipped、DESIGN.md §12 §2 Code Block Rendering 行）から変動しない。`--mermaid on` 時 / `auto` 検出時のみ +700 KB gzipped 程度の増加となる。この設計により、Mermaid を必要としないレビューフロー（仕様書 / 散文中心の文書）が肥大化を被らない。

`dist/standalone.html` には Mermaid runtime を build 時に inline する（設計判断 §5.l）。Shiki bundled 全言語と同じ「default で全部入り」配布契約を維持し、standalone を直接ダブルクリックで開いたユーザーも ` ```mermaid ` ブロックを SVG 描画で受け取れる。サイズ増分は +2.8 MB raw / +700 KB gzipped（現行 ~45 MB / gzip ~5.9 MB → ~48 MB / gzip ~6.6 MB）で許容範囲。
...
| `--mermaid` 値 | 挙動                                                                                                                                                                                                                                                          |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `auto`（既定） | `scanMermaidFences(markdown) > 0` のときだけ Mermaid.js を注入。0 件のときは注入せず配布物サイズも増えない。markdown が mermaid ブロックを含むかどうかに配布物サイズが連動する挙動で、配布者が markdown 内容を意識せずに CLI を回しても無駄な肥大化が起きない |
...
CLI 生成 `*-review.html`（`embed-template.html` ベース）と `dist/standalone.html` への Mermaid 注入後のサイズ見積もり：

| ケース                                                                   | 配布 HTML raw                         | embedded-mermaid 増分         | gzipped 後の配布物    |
| ------------------------------------------------------------------------ | ------------------------------------- | ----------------------------- | --------------------- |
| `*-review.html`: `--mermaid off` または `auto` で 0 件（後者が既定挙動） | ~327 KB（現行 `embed-template.html`） | 0 KB                          | ~99 KB（現行）        |
| `*-review.html`: `--mermaid on` または `auto` で 1 件以上                | ~327 KB                               | +2.8 MB raw / +700 KB gzipped | ~800 KB               |
| `dist/standalone.html`: Mermaid 同梱（build 時 default、§5.l）           | ~45 MB（現行）                        | +2.8 MB raw / +700 KB gzipped | ~6.6 MB（~48 MB raw） |
...
- Mermaid.js ESM build (`mermaid/dist/mermaid.esm.min.mjs`) を `<script id="embedded-mermaid" type="module">` で inline 注入したときに `globalThis.__mdxgMermaid` 経由で参照可能になること（§3.2 bridge 方式）、および `mermaid.render(id, src)` が同期 / async どちらで返るかを PoC で確認
...
- §10 Search との両立方針は **案 A（検索対象外）を確定採用済み（§4 Step 6）**。PoC では `[data-mermaid-applied]` 配下を `textSegments` で skip する実装が `<pre hidden>` の textContent / cmt mark 再貼付と干渉しないことを確認する

成果物：§5 マッピング表が確定状態、PoC で Mermaid ESM がブラウザで動くこと、§4 Step 6 §10 Search 案 A（検索対象外）skip 実装が干渉なく動くこと
...
### Step 3: ビルド側 — Mermaid ESM bundle の emit と standalone への inline
...
- `dist/mermaid.mjs` は **生成物だが commit 対象**（`dist/standalone.html` / `dist/embed-template.html` / `dist/review-request.mjs` / `dist/shiki-langs/` と同じ配布契約。clone 直後の利用者が `npm run build` 抜きで CLI / standalone どちらも実行できる）
- `package.json` に `mermaid` を `dependencies`（非 dev）として追加し、version を pin
- **standalone への inline**: `vite.config.ts` の `mdxg-split-outputs` plugin（あるいは新規 `mdxg-mermaid-asset` plugin）が closeBundle で `dist/mermaid.mjs` を読み、`</script>` escape を施した上で `dist/standalone.html` の `<script id="embedded-mermaid" type="module">` ブロックに inline する。CLI 経由（embed-template.html → \*-review.html）と standalone build で同じ bridge 注入規約（`globalThis.__mdxgMermaid` セット）を共有するため、ブラウザ側 `src/app/mermaid.ts` のロジック分岐は不要

成果物：`dist/mermaid.mjs` が emit され commit 対象になる + `dist/standalone.html` に Mermaid runtime が inline 済み
...
- §10 Search の挙動は **案 A（検索対象外）を採用**：upgrade 済み `<pre[data-mermaid-applied]>` 配下は `selection.ts` の `textSegments` が skip し、`collectSearchMatches` が回収しない。`<mark class="search-hl">` も貼られず、検索結果件数にも含まれない
  - 採用理由：Mermaid SVG 内 textContent は KaTeX / Shiki と異なり構造が予測不能（ノードラベル / arrow ラベル / subgraph タイトルが SVG `<text>` 要素として散らばる）で、ヒット位置の `scrollIntoView` 着地点を一貫させづらい。`<pre hidden>` を toggle する案 B は検索状態のライフサイクルと `data-mermaid-applied` の整合が複雑化（toggle 中に theme トグル再描画が走ったときの挙動など）。ダイアグラム DSL 内文字列を grep したい用途は実務上稀で、必要ならブラウザ標準の `Ctrl+F`（`<pre hidden>` の text node は DOM 上に残るためヒットする）で代替可能
  - 実装上の skip ポイントは `selection.ts` の `textSegments` 1 箇所に集約（`[data-mermaid-applied]` を子孫含めて skip）。`mark-engine.ts` の cmt mark 適用も `textSegments` を経由するため、SVG 化後のダイアグラムにはコメントも付かない（§6 と整合）
  - 未 upgrade 状態（Mermaid runtime 非注入時 / upgrade failed 時）の `<pre>`（`data-mermaid="1"` だけ付いていて `data-mermaid-applied` が無い）は **検索対象として残る**。Shiki ハイライト済みコードブロックとして表示されているため、検索の自然な対象になる
- in-source test に追加：
...
- DESIGN.md §13 ビルドパイプライン：`dist/mermaid.mjs` の出口を §13 全体像と表に追加し、`dist/standalone.html` が Mermaid runtime を default inline で同梱すること、CLI 経路（`embed-template.html` → `*-review.html`）は `--mermaid` モードで opt-in 注入することを併記
- DESIGN.md §1 概要 / §13 配布物の対応表：`dist/standalone.html` の grammar inline 説明に Mermaid 同梱を追記（Shiki と並列の「default で全部入り」要素として扱う）
...
| **C. paint 後 `requestIdleCallback` で lazy 初期化 + 各 `<pre>` upgrade** | ✓    | 初期描画は plain `<pre><code>` で即時 paint（Shiki ハイライトはそのまま乗る）。idle callback で `globalThis.__mdxgMermaid` から Mermaid インスタンスを取得（§3.2 bridge 方式）し、各 `<pre>` を SVG に upgrade する。Shiki upgrade と同じパターンで、既存 paint パスを変えずに追加機能を載せられる |
...
| 候補                 | 採用 | 理由                                                                                                                                                                                                                                                               |
| -------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`auto`（既定）**   | ✓    | `mermaid` ブロックを含まない markdown では注入が skip され配布物サイズが現行から変わらない。含む markdown のときだけ注入され配布物サイズが連動して増える挙動で、配布者が markdown 内容を意識せず CLI を回しても無駄な肥大化が起きない。`--shiki-langs auto` と同じ |
...
### l. `dist/standalone.html` に Mermaid を default で同梱する

| 候補                                                                                | 採用 | 理由                                                                                                                                                                                                                                                                                                                            |
| ----------------------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. `dist/standalone.html` に Shiki と同じく Mermaid を default inline**           | ✓    | Shiki bundled 全言語化と同じ「default で全部入り」配布契約を維持。standalone を直接ダブルクリックで開いたユーザーも ` ```mermaid ` を SVG 描画で受け取れる。サイズ増（+2.8 MB raw / +700 KB gzipped、~45 MB → ~48 MB、gzip ~5.9 MB → ~6.6 MB）は許容範囲。CLI 経路（`embed-template.html`）と挙動が分岐しないため UX が一貫する |
| B. CLI 経路（`*-review.html`）のみに `--mermaid` で opt-in 注入し standalone は外す | ✗    | standalone を直接開いたユーザーが ` ```mermaid ` を Shiki ハイライト済みコードブロックで読むことになり、CLI 経路と挙動が分岐する。「standalone は full feature」という配布契約から外れ、ユーザーが「描画されない」事故を踏みやすい                                                                                              |
...
- CLI 経路（`embed-template.html` → `*-review.html`）の `--mermaid <auto|on|off>` は維持する（§5.d / §5.e）。`embed-template.html` には Mermaid を default inline せず、CLI が markdown 内容に応じて注入する設計のままで、配布物サイズの最小化導線を残す。「standalone は full feature default / CLI 経路は配布物サイズ最適化のため opt-in」という役割分担で整合する
- standalone build と CLI 経路の bridge 注入規約（`<script id="embedded-mermaid">` + `globalThis.__mdxgMermaid`）は同一のため、ブラウザ側 `src/app/mermaid.ts` のロジック分岐は不要
- standalone build への inline は `mdxg-shiki-assets` plugin の closeBundle に倣って `mdxg-mermaid-asset` plugin（または既存 `mdxg-split-outputs` への統合）で `<script id="embedded-mermaid">` を埋め込む
...
- `app/shiki.ts`（既存テストに追加）：
  - `pre[data-mermaid="1"]` も通常通り Shiki upgrade 対象に含まれること（Mermaid runtime 非注入時のフォールバックとして Shiki ハイライトが効くことを保証）

- `app/code-copy-wrap.ts`（既存テストに追加）：
  - `pre[data-mermaid="1"]` には Copy button が注入されない

- `app/selection.ts`（既存テストに追加、§4 Step 6 案 A 確定採用）：
  - `textSegments` が `[data-mermaid-applied]` 配下を skip し、テキストセグメントとして返さない（cmt mark / search-hl 双方の貼付経路から構造的に外れる）
  - `data-mermaid="1"` だけ付いていて `data-mermaid-applied` が無い未 upgrade `<pre>` は通常どおりテキストセグメントとして拾われる（fallback Shiki ハイライト時の検索 / コメント対象として残る）

- `app/search.ts`（既存テストに追加）：
  - upgrade 済みダイアグラム内文字列は `collectSearchMatches` の戻り値に含まれず、`#search-count` の件数表示にも反映されない（§4 Step 6 案 A）
...
- `core/markdown.ts`（既存テストに追加）：
  - `code` トークンが `lang?.toLowerCase() === 'mermaid'` のとき、Shiki ハイライト経路に乗ったまま `<pre data-mermaid="1">` 属性付きで出力される（`Mermaid` / `MERMAID` のような大文字混入も同じ経路に乗ること）
...
- [ ] `dist/standalone.html` を直接ダブルクリックで開き、` ```mermaid ` ブロックが SVG として描画される（Mermaid build inline 経路の確認、§5.l）
- [ ] `dist/standalone.html` のサイズが見積もり通り（~48 MB raw / ~6.6 MB gzipped）
- [ ] §10 Search が mermaid 含み markdown でも動作する：upgrade 済み `<pre[data-mermaid-applied]>` 配下の文字列は検索対象外（件数にも含まれない）、未 upgrade（fallback Shiki ハイライト中）の `<pre[data-mermaid="1"]>` は検索対象として残る
...
- CLI が生成する `*-review.html`（`embed-template.html` ベース）のサイズが `--mermaid off` / `auto` で 0 件のときに **既存と変動なし**
- `*-review.html` の `--mermaid on` / `auto` で 1 件以上のときのサイズ増分が **gzip +800 KB 以内**
- `dist/standalone.html` のサイズが Mermaid inline 後 **gzip +800 KB 以内**（現行 ~5.9 MB → ~6.6 MB gzip 目安、§3.1 / §5.l）
- §6 アンカリングが壊れない（既存 in-source test 全通過 + 新規追加分も通過）
- §10 Search との干渉が案 A（検索対象外、§4 Step 6）で解決されている：upgrade 済みダイアグラムは検索 hit せず、未 upgrade `<pre>` は Shiki ハイライト済みコードブロックとして検索対象に残る
...
| §10 Search が mermaid ブロックで意図しない挙動になる                     | §4 Step 6 で案 A（検索対象外）を確定採用済み。`textSegments` で `[data-mermaid-applied]` 配下を skip する実装を Step 1 PoC で検証し、in-source test に regression ケース追加                                                 |
