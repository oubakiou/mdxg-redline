...
2. **helper**: 3 ファイル分割 — `src/core/i18n/i18n-core.ts` (純粋ロジック、Node/ブラウザ共通。Step 8 後の依存ルール整理で `app/i18n/` から `core/i18n/` へ移動) / `src/app/i18n/i18n-browser.ts` (ブラウザ副作用: `setLang` / `applyI18nDataset` / localStorage) / `src/cli/i18n.ts` (CLI state のみ、document/localStorage 非依存)
...
| ファイル                       | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 環境                |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `src/core/i18n/i18n-core.ts`   | **純粋ロジック**: `translate(dict, key, params?)` / `translatePlural(dict, { baseKey, count, params? })` / `detectLangFromEnv` / `detectLangFromNavigator` / `resolveInitialLang`。辞書は `MessageDict = Readonly<Record<string, string>>` で受ける (generic K を取らず `as K` unsafe assertion を回避)。**Step 8 後に `app/i18n/` から `core/i18n/` へ移動** (依存方向 `core ← app` / `core ← cli` の一方向不変条件を保つため、§13)                                                                                | Node / ブラウザ共通 |
...
### Step 6: (完了済み) 動的生成要素の言語切替対応

**状態**: **完了済み** — 本 Step の commit (本ファイルの状態行追記を同梱)。`src/app/document/load-document.ts` を **factory パターン** で新設 (12 tests)。`BootstrapDeps` に `documentLoader: DocumentLoader` (= `ResolvedBootstrapDeps`) を追加し、`resolveBootstrapDeps` で `createDocumentLoader(装飾済み loadFromMarkdown)` を組み立て。`boot.ts` / `paste-markdown-modal.ts` / `toolbar.ts` (Open file) の各 callsite を `runtime.documentLoader.loadDocument({kind, ...})` 経由に統一し、`BootRuntime` / `PasteMarkdownRuntime` / `ToolbarRuntime` の interface も `documentLoader` のみに置換。`source-display.ts` を DOM API ベース (`buildSourceLinkElement` で `createElement` / `appendChild` / `textContent`、innerHTML 経路撤去) に書き換え、`setupOnlineSourceI18n` (lang + document の 2 経路購読) / `teardownOnlineSourceI18n` の 4 関数パターンを実装。boot.ts の `showOnlineSource(result.finalUrl)` 直接呼び出しを撤去し、`registerOnDocumentLoad` hook 経由 (kind === 'online' で自動 show) に統一。`paste-markdown-modal.ts` の `showInputError` を `MessageKey` 引数に変更して `currentErrorKey` state 化、`setupPasteMarkdownModalI18n` / `teardownPasteMarkdownModalI18n` を追加。`page-navigation-render.ts` / `search-dom.ts` も `setup*I18n` / `teardown*I18n` で `subscribeLangChange` 経由 (`renderPageNavigation()` / `updateCountDisplay()`) の再描画を組み込み。`error-display.ts` の `showOnlineError(message)` を `showOnlineError(renderer: () => string)` 形式に変更し、`currentErrorRenderer` を保持して lang toggle 時に再評価 (`setupOnlineErrorI18n` / `teardownOnlineErrorI18n` 経由)。`app-wiring.ts` の bootstrap で `setupOnlineSourceI18n` / `setupOnlineErrorI18n` / `setupPageNavI18n` / `setupSearchI18n` / `setupPasteMarkdownModalI18n` を 1 度ずつ呼ぶ配線を追加。`vp check` / `vp test` (全 1408 tests + 4 skipped) 通過。
...
### Step 7: (完了済み) 既存設計との整合確認

**状態**: **完了済み** — 本 Step の commit (本ファイルの状態行追記を同梱)。`src/app/i18n/i18n-browser.ts` に「既存設計との共存 regression」describe ブロックを追加し、(a) lang toggle で `#doc` 外側の `data-i18n` 要素は翻訳され `#doc` 配下 (誤って `data-i18n` が混入したケースを含む) の `innerHTML` は完全に不変であること = `applyToElement` の `closest('#doc')` ガードが実際に発火する経路を踏む non-vacuous な test (DESIGN.md §6 アンカリング)、(b) `#doc` 配下の `mark.search-hl-current` の class / `data-search-index` 属性 (検索 current match の核心) が lang toggle で剥がされないこと (DESIGN.md §10 検索 current match との共存)、(c) `setLang` と本番 theme toggle 経路 `applyAppliedTheme` (`<html>.dark` class 更新) が双方向に独立 (lang→theme / theme→lang のどちらも互いを変更しない、DESIGN.md §1 Theming との独立) の 3 不変条件を test 3 件で固定。test (c) は theme module の `applyAppliedTheme` を実際に呼んで `.dark` class を toggle する本番経路を通す (dataset.theme 直接代入では theme.ts 経路を踏まないため避ける)。i18n-browser.ts の test は計 27 件 (新規 3 件 + 既存 24 件) で `vp test` 通過。DESIGN.md §9 起動シーケンスの paint 前 lang 解決は Step 5 で実装済み (`<head>` inline script + `bootstrapI18n`)。test 化されている不変条件は `bootstrapI18n` の `i18n-pending` class 解除 / `data-i18n` 翻訳 / idempotent の 3 点で、3 秒 timeout fallback と paint 前時系列そのもの (`applyI18nDataset` が body のレンダリング前に完了する保証) は実装はあるが test 化対象外、本 Step では追加実装なし。
...
- [DESIGN.md §12 MDXG 準拠状況と設計判断](./DESIGN.md#12-mdxg-準拠状況と設計判断) — UI 国際化は MDXG 規格外の独自拡張
