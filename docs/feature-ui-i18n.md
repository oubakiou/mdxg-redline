...
| ファイル                       | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 環境                |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `src/app/i18n/i18n-core.ts`    | **純粋ロジック**: `translate(key, params?, lang)` / `translatePlural(baseKey, count, params?, lang)` / `detectLangFromEnv` / `detectLangFromNavigator` / `resolveInitialLang`                                                                                                                                                                                                                                                                                                                                       | Node / ブラウザ共通 |
...
呼び出し例: `translatePlural('comments.count_label', 3)` → 内部で `'comments.count_label_other'` を MessageKey として lookup → 辞書値 `'{count} comments'` に `{count: 3}` を展開 → `"3 comments"`。
...
起動時に `applyI18nDataset(document)` が DOM を 1 回 walk し、各 dataset 属性別に対応するテキスト / 属性を置換する。**walk セレクタは `data-i18n*` 系の複合セレクタ** で、textContent 用 key だけでなく aria-label / placeholder / title / data-tooltip 専用 key を持つ要素も漏れなく拾う:
...
const SELECTOR = I18N_SELECTORS.map((s) => `${s}:not(#doc *)`).join(', ')
```
...
```ts
// applyI18nDataset の実装方針
export const applyI18nDataset = (root: Document | Element): void => {
  // root 自身が SELECTOR にマッチする場合も対象に含める (querySelectorAll は子孫のみ
  // なので、Step 6 の JS 動的経路で applyI18nDataset(#status) を呼んだ際に root 自身が
  // 翻訳されない問題を回避)。Document 渡しの初期描画では documentElement に i18n
  // 属性は付かないので Element 判定で十分。
  ...
  for (const el of targets) {
    if (el instanceof HTMLElement) {
      ...
      // placeholder 引数を持つ key 用 (data-i18n-params で JSON 経由で渡す)。
      // JSON.parse 失敗時は params なしで translate を呼ぶ (堅牢化)。
      let params: Record<string, string | number> | undefined
      if (dataset.i18nParams) {
        try {
          const parsed: unknown = JSON.parse(dataset.i18nParams)
      ...
      if (dataset.i18n) {
        if (import.meta.env.DEV && el.children.length > 0) {
          console.warn(
            `[i18n] data-i18n="${dataset.i18n}" on non-leaf element (children=${el.children.length}). Wrap text in a leaf <span data-i18n="..."> instead.`,
            el
          )
        }
        el.textContent = translate(dataset.i18n as MessageKey, params)
      }
      if (dataset.i18nAriaLabel)
        el.setAttribute('aria-label', translate(dataset.i18nAriaLabel as MessageKey, params))
      if (dataset.i18nPlaceholder)
        el.setAttribute('placeholder', translate(dataset.i18nPlaceholder as MessageKey, params))
      if (dataset.i18nTitle)
        el.setAttribute('title', translate(dataset.i18nTitle as MessageKey, params))
      if (dataset.i18nDataTooltip)
        el.setAttribute('data-tooltip', translate(dataset.i18nDataTooltip as MessageKey, params))
    }
  }
  // CSS 疑似要素用の custom property も更新 (§3.5 の CSS 疑似要素経路)
  document.documentElement.style.setProperty(
    '--ui-loading-text',
    `'${translate('empty.loading_text')}'`
  )
}
```
...
### Step 2: i18n 純粋ロジック層 + browser / CLI 分離実装
...
export function detectLangFromNavigator(language: string | undefined): Lang
export function resolveInitialLang(input: {
  storage?: string | null
  navigatorLanguage?: string | null
}): Lang
export function translate<K extends string>(
  dict: Record<K, string>, // 辞書を引数で受ける (UI 用 / CLI 用で別 dict)
  key: K,
  params?: Record<string, string | number>
): string
...
- 不正値の正規化: `translate` / `setLang` の lang 引数で `'fr'` 等の不正値は `'en'` fallback。env 値については §3.2 で別途規約: **空文字 / undefined は「未設定」として skip し次階層に fallback**、`'C'` / `'POSIX'` / `^ja(_|-|$)` 非マッチは `en` 確定 (POSIX セマンティクスに沿った 3 段評価)
...
// に戻されたケース (module 遅延成功) でも DOM と state が整合するよう、<html lang> を
// 再同期する。正常時 (head script で既に同じ値が設定済み) は idempotent な代入で no-op。
export const initLangFromBrowser = (): Lang => {
  let storage: string | null = null
  try {
    storage = readStoredLang()
  } catch (e) {}
  const navigatorLanguage = typeof navigator === 'undefined' ? null : navigator.language
  const lang = resolveInitialLang({ storage, navigatorLanguage })
  currentLang = lang
...
export const setLang = (lang: Lang): void => {
  ...
  // 配列をスナップショットしてから反復する (createDocumentLoader と同じ防御)。
...
// applyI18nDataset は [data-i18n*] 要素の更新に加え、CSS 疑似要素用の
// custom property (--ui-loading-text 等) も setProperty で更新する。
export function applyI18nDataset(root: Document | Element): void
export const translate = (
  key: keyof typeof messagesEn,
  params?: Record<string, string | number>
): string => translateCore(currentLang === 'ja' ? messagesJa : messagesEn, key, params)
export const translatePlural = (
  baseKey: PluralBaseKey,
  count: number,
  params?: Record<string, string | number>
): string =>
  translatePluralCore(currentLang === 'ja' ? messagesJa : messagesEn, baseKey, count, params)
```
...
```ts
import { translate as translateCore, type Lang } from '../app/i18n/i18n-core'
import { messagesCliEn } from './i18n/messages-cli.en'
...
export const translateCli = (
  key: keyof typeof messagesCliEn,
  params?: Record<string, string | number>
): string => translateCore(currentCliLang === 'ja' ? messagesCliJa : messagesCliEn, key, params)
```
...
### Step 4: CLI 自体の言語決定（HTML 側には作用させない）
...
- **不正値 / 値欠落の扱い** (セルフレビュー反映、`extractLang` がエラー情報を返す構造):
  ...
  - **後勝ち**: `--lang ja --lang en` → en、`--lang fr --lang en` → en (error クリア)、`--lang en --lang fr` → lang=en stays + error=invalid 'fr'（最後の有効値を保持しつつ最後のエラーも記録、`flag-parser.ts:150-178` の `assign: (acc, value) => ({ ...acc, field: value })` と整合）
...
### Step 5: HTML 側の起動シーケンスと toggle UI
...
- `src/app/i18n/i18n-browser.ts`（ブラウザ副作用、JSDOM fixture が必要）:
  ...
  - `applyI18nDataset(root)`: `data-i18n` / `data-i18n-aria-label` / `data-i18n-placeholder` / `data-i18n-data-tooltip` 各 dataset の更新、`#doc *` セレクタ除外、**`#doc [data-footnote-backref][data-i18n-aria-label]` の構造的例外包含**（§3.4 footnote.\* 行参照）、CSS custom property (`--ui-loading-text` 等) の setProperty 同時更新
  ...
  - 不正な `currentLang` 値（`'fr'` 等が誤って代入された場合）は en 辞書に fallback
...
- `src/cli/preextract-lang.ts` の `extractLang(argv, env)` （§3.3 二段階解析、サブパーサ非依存のグローバル メタフラグ処理）。`{ lang, argv, error }` を 1 パスで返す:
  ...
  - **後勝ち**:
    - `['--lang', 'ja', '--lang', 'en']` → `{ lang: 'en', error: null }`
    - **`['--lang', 'fr', '--lang', 'en']`** → `{ lang: 'en', error: null }` (invalid → valid 上書きで error クリア)
