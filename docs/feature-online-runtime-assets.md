...
| `cache`                        | 既に load 済みのアセットを追跡する in-memory state (`{ langs: Set<SupportedLang>, mermaid: boolean, katex: boolean }`)。省略時は module-level の global cache |
| 戻り値 `OnlineAssetLoadResult` | `{ loadedLangs: SupportedLang[]; mermaidLoaded: boolean; katexLoaded: boolean; failures: AssetLoadFailure[] }`                                                |
...
```ts
// src/app/online/runtime-decorator.ts (新規)
export const decorateRuntimeForOnline = (
  runtime: AppRuntime,
  cache: OnlineAssetCache
): AppRuntime => ({
  ...runtime,
  // 既存契約: loadFromMarkdown: (name, text) => Promise<void>
  // asset-loader だけ fire-and-forget、base runtime の Promise はそのまま return する
  loadFromMarkdown: (name: string, text: string): Promise<void> => {
    ...
    cache.generation += 1
    void loadOnlineAssets(text, getOnlineBaseUrl(), cache)
    return runtime.loadFromMarkdown(name, text)
  },
})

// src/app/app-wiring.ts (既存の app-wiring に追加)
const baseRuntime = createAppRuntime(...)
const runtime = isOnlineEdition()
  ? decorateRuntimeForOnline(baseRuntime, onlineAssetCache)
  : baseRuntime
toolbar.attach(runtime) // chrome/toolbar.ts は Open file change handler でこの runtime.loadFromMarkdown を呼ぶ
boot.attach(runtime)     // boot.ts は ?url= 経路でこの runtime を経由
openUrlModal.attach(runtime) // submit reload 経由なので実質 boot.ts と同じ経路
```

これで toolbar / Open URL / boot.ts の **どの入力経路でも同じ装飾 runtime** が使われ、asset-loader が一意に走る。decorator 関数自体は pure で、入力経路ごとに wrapper を散らさず DRY を保つ。詳細は §5.k。
...
### Step 2: Phase A.1 — asset-loader pure 関数 + renderer 後追い注入 API
...
- `src/app/online/runtime-decorator.ts` (新規 pure 関数、§5.k Item 3 対応): `decorateRuntimeForOnline(runtime, cache)` を export。app-wiring への組み込みは Step 4 (Phase A.3) で行うため、Step 2 ではエクスポートと in-source test のみ
...
- `src/app/app-wiring.ts`: `isOnlineEdition()` true なら `decorateRuntimeForOnline(baseRuntime, createOnlineAssetCache())` を一度だけ呼んで装飾、toolbar / boot / Open URL modal すべての attach 先に同じ装飾 runtime を渡す
...
| 候補                                                                                 | 採用 | 理由                                                                                                                                                                   |
| ------------------------------------------------------------------------------------ | ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. `decorateRuntimeForOnline(runtime, cache)` を `app-wiring.ts` で 1 度だけ適用** | ✓    | toolbar (Open file) / Open URL modal (reload 経由) / boot.ts (?url=) の **全入力経路** が同じ装飾 runtime を経由。経路追加時の見落としが構造的に起きない (Item 3 対応) |
...
- **decorator 関数の pure 性**: `decorateRuntimeForOnline(runtime, cache)` は副作用ゼロの pure 関数で、入力 runtime を変更せず新オブジェクトを返す。in-source test で base runtime が mutate されないことを検証
...
- **CLI 経路 (standalone / embed-template) での影響ゼロ**: `isOnlineEdition()` false なら装飾せず baseRuntime をそのまま使う。standalone / embed-template の挙動は完全に不変
- **新入力経路の追加への耐性**: 将来の input 経路 (例: drag & drop / clipboard paste / 別 modal) も `runtime.loadFromMarkdown` を呼ぶ限り asset-loader が走る。decorator は app-wiring の 1 点で適用される構造的不変条件
- **Phase 分割への影響**: Phase A.1 の asset-loader 実装と一緒に decorator も export、Phase A.3 で app-wiring に組み込む。Phase A.1 だけだと decorator が使われずに end-to-end で動かないため、A.1 と A.3 のスコープ境界に注意
...
- `app/online/runtime-decorator.ts` (新規、§5.k Item 3 対応):
  - `decorateRuntimeForOnline`: base runtime を mutate しない (pure 関数の不変条件) / 装飾 runtime の `loadFromMarkdown` 呼び出しで asset-loader が `void` で fire-and-forget 発火 + base.loadFromMarkdown の `Promise<void>` を **return** する (await chain を壊さない)
...
- `app/app-wiring.ts` (既存テストに追加):
  - `isOnlineEdition()` true なら `decorateRuntimeForOnline` が 1 度だけ呼ばれて装飾 runtime が toolbar / boot / openUrlModal の全 attach 先に同じ instance で渡される
...
| **Open file 経路が boot.ts wrapper を通らない** (`chrome/toolbar.ts:219` が直接 runtime を呼ぶ、Item 3)                              | §5.k で確定: `app-wiring.ts` で `decorateRuntimeForOnline` を 1 度だけ適用し、toolbar / boot / Open URL modal の全 attach 先に装飾 runtime を配る。boot.ts inline wrapper は不可、入力経路追加時の見落としを構造的に防ぐ                        |
