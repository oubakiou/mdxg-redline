...
| ファイル                   | 用途                                                       | grammar / Mermaid inline                                                    | 開く対象                             |
| -------------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------------- | ------------------------------------ |
| `dist/standalone.html`     | 単独 Open file 用、エンドユーザーが直接開く                | Shiki bundled 全言語（約 235） + Mermaid runtime をいずれも事前 inline      | ユーザーが直接ダブルクリック         |
| `dist/embed-template.html` | review-request CLI が rewrite して配布 HTML を生成する素材 | grammar / Mermaid いずれも 0（CLI が `--shiki-langs` / `--mermaid` で注入） | CLI 経由でのみ使用、直接開く想定なし |
...
```
node dist/review-request.mjs [--no-open] [--show-open-file] [--document-name <name>] [--theme <system|light|dark>] [--shiki-langs <auto|all|none|<csv>>] [--mermaid <auto|on|off>] [--comments-width <0|280-640>] [--page-nav-width <0|180-480>] [--markdown-css <path>] <input.md|-> [output-dir]
```
...
- ビルドパイプラインは開発者ローカルでのみ動作。配布物 (`dist/standalone.html` / `dist/embed-template.html` / `dist/review-request.mjs` / `dist/shiki-langs/*.json` / `dist/mermaid.mjs`) はすべてビルド成果物としてリポジトリにコミットされており、エンドユーザー環境にはツールチェーンを持ち込まない
...
vite.review-request.config.ts ─┘                          shebang 付き、
                                                          embed-template.html / mermaid.mjs を読む)
```

**`mdxg-split-outputs` plugin**: viteSingleFile が中間出力 `dist/review.html` を生成した後、`mdxg-shiki-assets` の closeBundle で Shiki bundled 全言語（約 235）の grammar JSON が emit され、続いて本 plugin の closeBundle が走る。役割は次の 3 点：
...
3. 中間出力 `dist/review.html` を `dist/embed-template.html` にリネーム（こちらは grammar / Mermaid runtime いずれも注入なし、CLI が `--shiki-langs` / `--mermaid` モードに応じて動的に注入する）

なお `dist/mermaid.mjs` は `vite.mermaid.config.ts` 側で別途生成されるため、`npm run build` script は `vp build --config vite.mermaid.config.ts && vp build && vp build --config vite.review-request.config.ts` の順で実行する（mermaid → standalone inline → CLI bundle の依存順）。

**配布物サイズ実測（Mermaid 同梱影響）**: `dist/mermaid.mjs` ~3.1 MB raw / gzip ~859 KB。`dist/standalone.html` は Mermaid inline を含めて ~48 MB raw / gzip ~6.7 MB（Shiki only 比 +700 KB gzipped）。CLI 生成 `*-review.html` は `--mermaid off` または `auto` × 0 件で ~100 KB gzipped（`embed-template.html` 同等）、`--mermaid on` または `auto` × 1 件以上で ~800 KB gzipped（+700 KB）。「`mermaid` ブロックを含まない markdown はサイズが現行と変わらず、含む markdown だけ自動で runtime が乗る」配布物サイズ最適化の核となる挙動 (§12 §15 / §3 `--mermaid`)。
...
| 出力分岐                    | mdxg-split-outputs     | closeBundle で `dist/shiki-langs/*.json` 27 ファイルを読み、`<script id="embedded-shiki-langs">` に inline した `dist/standalone.html` を書き出す。中間出力 `dist/review.html` は `dist/embed-template.html` にリネーム。`dist/mermaid.mjs` が存在すれば併せて `<script id="embedded-mermaid" type="module">` に inline する（不在時は警告して skip、CLI 経路でのみ runtime が注入される fallback 構成） |
...
```bash
# 1 回ビルド（commit 前に必ず叩く）。mermaid.mjs / standalone.html / embed-template.html / review-request.mjs を生成する。
npm run build                  # = vp build --config vite.mermaid.config.ts && vp build && vp build --config vite.review-request.config.ts
...
`npm run build` 後に `dist/standalone.html` / `dist/embed-template.html` / `dist/review-request.mjs` / `dist/mermaid.mjs` が再生成される。ソースと生成物の同期は人手 + 任意で pre-commit hook で担保する。`dist/review-request.mjs` は実行時に同ディレクトリの `dist/embed-template.html` を読み込み、`--mermaid` 経路では `dist/mermaid.mjs` も読み込むため、これら 4 つは常に揃った状態で commit する。
...
- `core/embed.ts`：markdown を JSON 文字列にエンコード（`<` を `<` に置換）、`data-name` 属性エスケープ、`<script id="embedded-md" type="text/markdown">` の rewrite、`<script id="embedded-mermaid" type="module">` への Mermaid runtime 注入 + `</script>` escape 件数返却。Node CLI からもブラウザ側からも再利用できる pure module
- `core/scan-mermaid.ts`：marked.lexer ベースで markdown 内の ` ```mermaid ` フェンス数を数える純粋関数 (CLI `--mermaid auto` の判定中核)
- `cli/parse-args.ts`：CLI 引数パース (`--no-open` / `--document-name` / `--mermaid <auto\|on\|off>` / stdin token `-`) と出力ファイル名サニタイズ
...
- `cli/review-request.ts`：CLI エントリの run 関数（引数 → embed → 書き出し → ブラウザ起動の orchestration） + `shouldInjectMermaid` (mode × scanMermaidFences の純粋判定)
...
1. ソースは `src/` 配下（`cli/` / `app/` / `core/` / `styles/` / `review.html` / `mermaid-entry.ts`）のみを編集する
2. ビルド出力（`dist/standalone.html` / `dist/embed-template.html` / `dist/review-request.mjs` / `dist/shiki-langs/*.json` / `dist/mermaid.mjs`）は **手で編集しない**（次の `vp build` で上書きされる）
...
- **`src/core/`** — 環境非依存の pure module。Node CLI / ブラウザ双方から import される（markdown / block-anchors / page-split / page-outline / slugify / search / embed / escape / feedback / review-export / scan-fenced-langs / scan-mermaid / shiki-aliases.generated / types）
- **`src/app/`** — Browser DOM / Web API 専用のランタイム（review エントリ / boot / app-state / doc-renderer / mark-engine / selection / comment-modal / floater / comments / toolbar / workspace / theme / search / scroll-spy / page-navigation / mermaid / mermaid-modal / `*-width` / `*-resize` 等）
- **`src/cli/`** — Node CLI 専用（review-request エントリ / parse-args / clean / input-source / open-command / serve）

エントリ素材は `src/review.html` / `src/styles/*.css` / `src/mermaid-entry.ts` (vite.mermaid.config.ts の入口、`globalThis.__mdxgMermaid` bridge をエクスポート)。`app` / `cli` の型を `core` に持ち込まない。`dist/` 配下（`standalone.html` / `embed-template.html` / `review-request.mjs` / `shiki-langs/*.json` / `mermaid.mjs`）は `vp build` の生成物で、commit 対象だが手では編集しない。
