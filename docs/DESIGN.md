...
ホスティング先 (例: Cloudflare Pages) で `dist/hosting/index.html` (build pipeline の C 設計、§13) が配信されているとき、`?url=https://...` クエリで外部 markdown を fetch して描画する経路。standalone.html / embed-template.html ではこの経路は無効化されている（§11.b 配布物の境界）。
...
| `--math-fonts <minimal\|all>`            | KaTeX 注入時のフォント family 範囲を制御。`minimal` は Main / AMS / Math / Size1〜4 の 9 family のみ data URI inline（gzip +約 240 KB）、`all` は `\mathcal` / `\mathfrak` / `\mathscr` / SansSerif / Typewriter を含む 20 family を inline（gzip +約 340 KB）。`--math off` のときは黙って ignore。詳細 §12 §14                                                                                                                                             | `minimal`        |
...
幅 ≤ 768px のスマートフォンでは、3 列 grid を `@media (max-width: 768px)` ブロックで上書きし、**上端固定ヘッダ + 画面下端の fixed footer バー + 左右 drawer** モデルに切り替える。ヘッダ自体は `position: fixed` ではなく、`body` を `overflow: hidden` + `height: 100dvh` で非スクロール化し static なヘッダを上端に pin した上で `.doc-pane` のみをスクロールコンテナにする（footer / drawer / backdrop は `position: fixed`）。header は `Open ▾` / `⚙ Settings` のみ残し、footer の 3 ボタン (TOC / Search / Comments) が左右 sidebar (`.page-nav` / `.comments`) を `position: fixed` + `transform` で slide-in する drawer の開閉 trigger になる。drawer 開閉状態は `<html>` の `mobile-page-nav-open` / `mobile-comments-open` class で表現し (`src/app/chrome/mobile-footer.ts`)、既存 desktop の `*-closed` 系 (grid 列幅制御) とは直交させる。背面は `inert` + scroll lock で保護し、Tab focus は drawer + footer 内に循環する。範囲選択時の `#floater` (＋ Comment) は、mobile では選択 rect 追従のポップオーバーではなく `bottom: var(--mobile-footer-height)` で footer 直上に固定したフル幅バーに化け、選択範囲に張り付くブラウザネイティブの選択メニューと構造的に重ならない (`docs/feature-mobile-layout.md` §5.t)。safe-area inset / 動的 viewport は `--mobile-header-height` / `--mobile-footer-height` 等の CSS custom property に集約し、`viewport-fit=cover` で iOS の inset を受け取る。769–900px は既存タブレット (vertical-stack)、901px 以上は desktop モデル。
...
| localStorage                      | `mdxg-redline.theme` / `.comments-width` / `.comments-open` / `.page-nav-width` / `.page-nav-open` | enum / 整数 (px)                  | theme・コメントパネル・左 TOC の幅と開閉状態。FOUC 防止のため `<head>` 内 inline script が同期で読む |
...
-1. [<head> inline script、paint 前] theme / lang 初期解決
    ...
    - lang: localStorage('mdxg-redline.lang') > navigator.language > 'en' (§14.3)
            ...
            (lang は theme と逆順で localStorage が最優先。CLI は HTML への lang 属性を
             一切埋め込まない CLI/HTML 責務分離方針のため、§14.1 / §14.3)
...
2. 埋め込み markdown (<script id="embedded-md">)
   ...
   2d.5 [online edition のみ] loadOnlineAssets(markdown, baseUrl, cache) を fire-and-forget
       ...
       - renderers/{shiki,mermaid,katex}-upgrade.ts の永続 listener (online edition
         でのみ attach、§13) が dispatchEvent を pickup して scheduleXUpgrade を再走、
...
幅 ≤ 768px のスマートフォンでは、3 列 grid の代わりに上端固定ヘッダ + 画面下端の fixed フッターバー + 左右 drawer モデルに切り替わる（`viewport-fit=cover` で iOS safe-area inset を尊重）。769–900px は既存タブレット（vertical-stack）、901px 以上は desktop モデル。
...
**なぜ runtime アセット (Shiki grammar / Mermaid / KaTeX) を公開 CDN ではなく同一オリジン同梱にするか**: jsdelivr / unpkg 等の公開 CDN から取得する設計を採ると、(1) `connect-src` allowlist に外部 origin を追加する必要があり信頼境界が広がる、(2) CDN 障害で online edition が同時に壊れる単一障害点が生まれる、(3) 外部依存が「単一 HTML + 外部依存ゼロ」原則 (§13 / standalone 経路と共有) と衝突する。同一オリジン同梱なら `'self'` 1 つで全アセットがカバーされ、hosting 側の deploy 単位とアセット供給単位が一致するため deploy 世代ずれの fail-safe (§13 / archive doc §5.i の fingerprinted/canonical 3 段) も構造的に成立する。容量面の合計サイズは Cloudflare Pages の per-deploy 上限内に十分収まる (Pages 無料枠は 20,000 ファイル、各 grammar JSON は ~200 KiB)。
...
ビルドパイプラインは開発者ローカルでのみ動作。配布物はすべてビルド成果物としてリポジトリにコミットされており、エンドユーザー環境にはツールチェーンを持ち込まない。配置先は **CLI / Releases 配布用 (dist 直下)** と **Pages 配信用 subset (`dist/hosting/`)** で物理的に分離される (online edition の最小 shell 化に伴う設計、archive 化された `docs/archive/feature-online-runtime-assets.archive.md` Step 4 / §13)：
...
- [MUST] ライト / ダーク両ホストテーマ対応: ✓（toolbar の歯車ボタン (⚙) から設定モーダルを開き、`Theme` select で `system` / `light` / `dark` を選択。歯車ボタンに `aria-label="Settings"` / `data-tooltip="Settings"` を付与）

優先順位 P1: `<html data-theme>` (CLI `--theme` ヒント / §3) > `localStorage('mdxg-redline.theme')` > `prefers-color-scheme` (詳細は §7c)。dark の primary button は `#2cac6e` × `#ffffff` が 2.77:1 で AA 不足のため文字色を `#1a1a1a` に反転 (`src/styles/review.css` 参照、Material / Tailwind dark の慣行と一致)。dark トークン値は DADS 公式 dark semantic 未公開のため primitive から自前マッピング（公開時に差し替え予定）。
...
- **Stacked View**: doc-renderer は markdown 読み込み時に全 page を `<section class="virtual-page" data-page-index data-page-slug>` で 1 度に描画する。マウスホイールだけで全文を読み進められる Word 風レイアウト (docs/archive/mdxg-virtual-pages.archive.md §14)。blockId は document スコープ連番に戻り (§7.1 撤回)、mark-engine は全 comments を活性 mark として保持する。mobile (≤768px) では `.doc-pane` に `scroll-snap-type: y proximity` + `.virtual-page` に `scroll-snap-align: start` を当て、上下フリックで指を離した位置がページ境界近傍のとき先頭へ吸着する (連続通読を壊さないため `proximity`、`scroll-padding-top: 5%` は §6-7 の TOC / hash ジャンプの着地点と揃える。詳細と「1 フリック=1 ページ」要望への達成度ギャップは `docs/feature-mobile-layout.md` §5.u)
...
- **狭幅レスポンシブ**: `@media (max-width: 900px)` で page-nav 列を隠す (769–900px タブレット域の vertical-stack)。幅 ≤ 768px のスマートフォンは drawer モデルに切替 (§4 モバイルレイアウト / §10)
...
- **toolbar の Help (?) ボタン**: app-header 右側の歯車ボタン (Settings) の隣に `<button id="btn-help" class="btn btn-ghost tooltipped">` を置き、クリックで `toggleHelpModal()` を呼ぶ。`aria-label="Show keyboard shortcuts"` + `data-tooltip="Keyboard shortcuts (h)"` で「`h` キーでも開ける」ことを示す。toolbar に置くのは「TOC が closed のときも常時可視であってほしい」要件を満たすため (TOC ヘッダー内に置くと closed 時に見えなくなる)
...
- [SHOULD] ホスト色スキーム適応: ✓（`resolveThemeVariables()` で DESIGN.md §1 Theming の CSS variables (`--paper` / `--ink` / `--accent` / `--rule` / `--doc-code-bg`) を Mermaid `themeVariables` 形式に写像、`subscribeSystemTheme` callback と設定モーダルの theme select 変更の双方から `redrawMermaidForTheme` を呼んで全 SVG を破棄 → 再 upgrade。Mermaid は CSS variables を直接読まず initialize 時の値を SVG に焼き込むため、CSS だけでは追従できない）
...
- **モバイル UI の残課題**：≤768px の主要 chrome（上端固定ヘッダ + footer バー + 左右 drawer + search-bar、navigation 経路の drawer 自動 close、Edit modal trigger の drawer 自動 close、focus 復元契約、breakpoint 切替時の focus 退避）に加え、範囲選択時の floater を footer 直上のフル幅固定バー化してネイティブ選択メニューとの重なりを回避する対応も実装済み（§4 モバイルレイアウト / §10、`docs/feature-mobile-layout.md` §5.t）。次フェーズ候補として以下が残る：
  - **選択ハンドル制御**：モバイルのテキスト選択 UX に合わせた選択ハンドルの最適化（floater 位置はバー化で対応済み）
  - **drawer 内部 UI の密度最適化**：モバイル特化の TOC 折りたたみ・コメントカード密度調整（主要 chrome 配置の次フェーズ）
  - **`.cmt-del` 即時削除後の focus 消失対処**：削除後に再描画される cmt-list で focus が消える既存問題（desktop / mobile 共通）。将来 `restoreFocusAfterClose` 相当を `deleteComment` 後に呼んで双方改善する
  - **mobile での Help モーダル動線**：`#btn-help` を mobile で hide しているため、タッチ専用環境では Help を開けない（Bluetooth キーボード `h` キーは動作）。専用 footer overflow メニュー等は将来検討
  - **RTL レイアウト**：drawer の slide 方向（`translateX(±100%)`）は `dir=ltr` 前提。RTL 言語を i18n に追加する際に方向反転と border / padding の左右切替を同時対応
  - **PWA / `display-mode: standalone`**：A2HS / manifest による PWA 化と PWA 起動時の status bar / safe-area 挙動
  - **単一ページ文書での TOC ボタン hide**：現状は文書未読込時のみ `#btn-mobile-toc` を hide（`has-pages` フラグは単一ページ文書でも付く）。「ページ分割が無い文書で TOC を隠す」には `data-page-count` 等の属性追加が必要
  - **landscape 左右 safe-area inset の実機検証**：drawer モデル対象機種に notch がなく実機検証経路が無いため、現状は予防的設計（`calc(... + env(safe-area-inset-*))`）のみ。Android Foldable 等への対応時に検証
  - **別ページ遷移の再描画コスト（性能 bug、解決済み）**：TOC からの別ページ遷移で `navigateToTarget` が `#doc` 全体を再 mount していた性能 bug は、ページ切替経路を軽量 `refreshActivePageView`（TOC highlight 更新のみ）に変更して解決済み（commit `2f88119`）。経緯は [docs/archive/bug-stacked-view-pagechange-rerender.archive.md](archive/bug-stacked-view-pagechange-rerender.archive.md)
...
ビルドの出口は 4 つ。エンドユーザーが直接開く配布物 `dist/standalone.html`、review-request CLI が rewrite テンプレートとして読み込む `dist/embed-template.html`、ホスティング配信用の `dist/hosting/index.html`（URL fetch viewer、§3 入力 3）、配布者向け CLI ツール `dist/review-request.mjs`。加えてホスティング先 (Cloudflare Pages 等) 用の静的設定ファイル `dist/hosting/_headers` が build 時に emit される (Cloudflare Pages の Build output directory に `dist/hosting` を指定する設計、archive 化された `docs/archive/feature-online-runtime-assets.archive.md` Step 4)。
...
| `dist/hosting/index.html`                                                                        | C1   | ホスティング配信用の URL fetch viewer（§3 入力 3）。最小 shell として grammar / Mermaid / KaTeX runtime はすべて空 textContent / 空 style に上書き済みで、`<script id="online-asset-manifest">` 経由で runtime が必要分だけ動的取得する（archive 化された `docs/archive/feature-online-runtime-assets.archive.md` 参照） | ~402 KB / gzip ~124 KB             |
...
7. `dist/standalone.html` から `buildOnlineHtml(standaloneHtml, { allowlist, manifest })` で `dist/hosting/index.html` を派生 (Pages 慣習に合わせて `online.html` → `index.html` リネーム、archive 化された `docs/archive/feature-online-runtime-assets.archive.md` Step 4 の C 設計判断)。次の 8 mutation を apply: (a) `<html data-mdxg-online="1">` upsert、(b) CSP `connect-src 'none'` → `connect-src 'self' <allowlist origins>` 置換 (`'self'` は同一オリジン同梱資材 fetch / dynamic import 用、§11.b)、(c) `<script id="online-allowlist">` JSON 注入、(d) `<script id="online-asset-manifest">` JSON 注入 (manifest 経由で loader が fingerprinted パスを解決、archive doc §3.2)、(e) `<script id="embedded-shiki-langs">` の textContent を空 `{}` に上書き、(f) `<script id="embedded-mermaid">` の textContent を空に上書き、(g) `<script id="embedded-katex">` の textContent を空に上書き、(h) `<style id="embedded-katex-css">` / `<style id="embedded-katex-fonts-extra-css">` の textContent を空に上書き (素材契約として block 不在は fail-fast、Shiki と対称)。これにより `dist/hosting/index.html` が「最小 shell」となり、必要なアセットはすべて runtime fetch / dynamic import で動的取得される。allowlist は `buildOnlineAllowlist(process.env)` で env var `MDXG_ONLINE_CONNECT_SRC` を読んで正規化、解決済み allowlist を stdout に emit して build 再現性を保証する
8. `buildOnlineHeadersFile(onlineHtml)` で `dist/hosting/_headers` を emit（Cloudflare Pages 用静的設定。 index.html の meta CSP と HTTP header CSP は `extractCspContent(onlineHtml)` 経由で single source of truth、§11.b）。`_headers` の Cache-Control は path 単位で 3 分離: `/` / `/index.html` は `max-age=300`、`/fingerprinted/*` は `immutable, max-age=31536000`、`/canonical/*` は `max-age=300`。 `_redirects` は不要 (Pages が `/` request に対して自動的に `index.html` を返すため)
...
| Shiki aliases 再生成        | mdxg-shiki-aliases     | buildStart で `src/core/shiki-aliases.generated.ts` を再生成 (Shiki `bundledLanguagesInfo` から 全正規名 + ALIAS_TO_CANONICAL を抽出)。**grammar JSON の emit はこの plugin では行わない** (closeBundle parallel race を避けるため、grammar emit は `mdxg-split-outputs` に集約)。共通ロジックは `scripts/lib/shiki-meta.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 出力分岐                    | mdxg-split-outputs     | closeBundle 内で逐次 await: ① `emitGrammarJsonFiles()` で grammar JSON を 3 系統 (CLI / hosting/canonical / hosting/fingerprinted) に emit + AssetEmission を組み立て。② `emitMermaidRuntimeFiles()` で C2 出力を hosting fingerprinted/canonical の 2 系統に hash 焼き込み emit。③ `emitKatexAssetFiles()` で C3 出力 3 ファイルを同じく 2 系統 emit。④ 中間出力 `dist/review.html` を読み、⑤ `<script id="embedded-shiki-langs">` に全 grammar を inline + `dist/mermaid.mjs` / `dist/katex/*` が揃っていれば各 embed ブロックに inline (不在時は警告 skip) して `dist/standalone.html` を書き出す。⑥ standalone から `buildOnlineHtml(standaloneHtml, { allowlist, manifest })` で `dist/hosting/index.html` を派生（最小 shell の 8 mutation: `data-mdxg-online` / CSP `'self'` prepend / `<script id="online-allowlist">` / `<script id="online-asset-manifest">` / 空 `<script id="embedded-shiki-langs">{}` / 空 `<script id="embedded-mermaid">` / 空 `<script id="embedded-katex">` / 空 `<style id="embedded-katex-css">` + `<style id="embedded-katex-fonts-extra-css">`）。⑦ Cloudflare Pages 用 `dist/hosting/_headers` (CSP + path 単位 Cache-Control) を emit (`_redirects` は不要、 Pages が `/` で自動的に index.html を返す)。⑧ 中間出力 `dist/review.html` を `dist/embed-template.html` にリネーム |
...
| レイヤー        | ツール                | 役割                                                                                                                                                                                                                                                                                                                   |
| --------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| バンドル        | Rolldown（Vite 内蔵） | `src/cli/review-request.ts` を入口に `src/cli/{arg-spec,parse-args,parse-clean-args,parse-run-args,input-source,open-command,serve}.ts` と `src/core/embed.ts` + `src/core/filename-sanitize.ts` + `src/core/math.ts` を 1 つの ESM (`dist/review-request.mjs`) に統合。Node 組み込みモジュール (`node:*`) は external |
...
CLI と HTML は **言語決定について疎結合に保つ** (§14.1 / §14.3)。CLI は HTML への lang 関連属性 (旧設計案の `<html data-lang-init>` 等) を一切埋め込まず、HTML 側は `localStorage > navigator.language > 'en'` で独立に決定する。`src/cli/compose-review-html.ts` の rewrite チェーンには lang 関連の upsert を追加せず、CLI 自身の `--lang` (§14.4) は `setCliLang` 経由で stdout / stderr / help 出力にのみ作用させる。

辞書も **CLI bundle と HTML bundle で物理分割** する (§14.2)：

- CLI bundle (`dist/review-request.mjs`) は `src/cli/i18n/messages-cli.{en,ja}.ts` (`cli.*` 約 36 entry) のみ import
- HTML bundle (`dist/standalone.html` / `dist/embed-template.html` / `dist/hosting/index.html`) は `src/app/i18n/messages.{en,ja}.ts` (UI 辞書 約 155 entry、`online.*` / `footnote.*` 含む) のみ import

それぞれ独立した import tree なので bundle 重複なし。`dist/hosting/index.html` は `dist/standalone.html` から派生 (`buildOnlineHtml`、上の `mdxg-split-outputs` plugin の手順 ⑥) するため、UI 辞書は派生元に既に inline 済みで online 派生処理での辞書再 inject は不要。
...
| 判断項目           | 採用                                                                 | 理由                                                                                                                                                                                                         |
| ------------------ | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 言語切替の手段     | ランタイム設定モーダル内 select + CLI 自体は CLI 出力のみ制御        | CLI と HTML を疎結合に保ち、CLI 生成版と online 版で言語決定ロジックが揃う。レビュー依頼者がレビュワーの初期言語を制御する経路は提供しないが、初回選択後は localStorage で永続化されるため小さなトレードオフ |
| HTML 側の初期言語  | `localStorage > navigator.language > 'en'` の 3 層                   | ユーザーの過去選択 > ブラウザ環境 > 最終 fallback の自然な序列。CLI 生成版 / online 版で共通の規則になる                                                                                                     |
| CLI 自体の初期言語 | `--lang` フラグ > `$LC_ALL` > `$LC_MESSAGES` > `$LANG` > `'en'`      | POSIX Locale 階層に整合。`$LC_MESSAGES=ja_JP.UTF-8 $LANG=en_US.UTF-8` のような「messages カテゴリだけ ja」構成 (CI / 個人 dotfile で珍しくない) でも CLI が日本語表示される                                  |
| 辞書ライブラリ     | 素朴 object + 自作 `translate()`                                     | 約 195 entry に対して `i18next` (30+ KB gzip) / `intl-messageformat` (10 KB) は過剰。配布物サイズ + 依存をゼロに抑える                                                                                       |
...
| キー命名規約       | ドット区切り名前空間 (`toolbar.open_file`)                           | 約 195 entry で grep / 整理がしやすい、未使用 key 検出も簡単。`keyof typeof messagesEn` で型推論が効く                                                                                                       |
| 辞書値の信頼境界   | textContent / attribute 経路でのみ書き込み、innerHTML には注入しない | XSS 経路を構造的に閉じる。`data-i18n-params` の user 由来文字列 (docName 等) は JS 動的経路 (`dataset.i18nParams = JSON.stringify(...)`) で設定し、ブラウザの dataset setter が自動 escape する (§11.a)      |

### 14.2 翻訳辞書とランタイム

辞書を **UI / CLI で物理分割** することで bundle 重複を避け、ランタイム実装も `document` / `localStorage` 依存の有無で **3 モジュールに分割** (両環境共通 / ブラウザ専用 / Node 専用) する：

| ファイル                          | 内容                                                                                                                                                                                                        | 環境                |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `src/app/i18n/messages.en.ts`     | UI 辞書 en (約 155 entry)。`MessageKey = keyof typeof messagesEn` を export                                                                                                                                 | ブラウザ用 bundle   |
| `src/app/i18n/messages.ja.ts`     | UI 辞書 ja。`satisfies Record<MessageKey, string>` で key 一致を tsc レベル保証                                                                                                                             | 同上                |
| `src/cli/i18n/messages-cli.en.ts` | CLI 辞書 en (`cli.*` 約 36 entry、help は block 形式)                                                                                                                                                       | CLI bundle のみ     |
| `src/cli/i18n/messages-cli.ja.ts` | CLI 辞書 ja (`satisfies Record<CliMessageKey, string>`)                                                                                                                                                     | 同上                |
| `src/core/i18n/i18n-core.ts`      | 純粋ロジック。`translate(dict, key, params?)` / `translatePlural(dict, options)` / `detectLangFromEnv` / `detectLangFromNavigator` / `resolveInitialLang`                                                   | Node / ブラウザ共通 |
| `src/app/i18n/i18n-browser.ts`    | ブラウザ副作用。`setLang` / `getLang` / `initLangFromBrowser` / `subscribeLangChange` / `applyI18nDataset` / `translate` / `translatePlural` の wrapper。`<html lang>` / `localStorage` / `document` を操作 | ブラウザ専用        |
| `src/cli/i18n.ts`                 | CLI state。`setCliLang` / `getCliLang` / `translateCli`。`document` / `localStorage` には触らない                                                                                                           | Node 専用           |

online 版 (`dist/hosting/index.html`) は standalone HTML から派生して生成され、UI 辞書は派生元に既に inline 済みなので **online 版での辞書再 inject は不要**。

### 14.3 言語決定の優先順位

CLI 自体と review HTML は **責務分離** し、互いに影響しない。CLI は HTML 側に lang 関連属性 (旧設計案の `<html data-lang-init>` 等) を一切埋め込まない：
...
`--lang <auto|en|ja>` を **サブパーサ非依存のグローバル メタフラグ** として扱い、bootstrap で argv strip + lang 抽出 + エラー検出を 1 パスで行う (`src/cli/preextract-lang.ts:extractLang`)。サブパーサ (`parse-run-args` / `parse-clean-args`) は `--lang` の存在を知らずに動く。

| 値     | 挙動                                                                         |
| ------ | ---------------------------------------------------------------------------- |
| `auto` | `$LC_ALL` / `$LC_MESSAGES` / `$LANG` から推定。env も空なら `en` で CLI 出力 |
| `en`   | CLI の help / stderr を英語で出力                                            |
| `ja`   | CLI の help / stderr を日本語で出力                                          |

不正値 (`fr` / `spec.md` 等) は **二段階解析** で扱う：(a) `extractLang` が `{lang, argv, error}` を返し、(b) `main()` 側で「help 最優先 → lang エラー reject → 通常モード判定」の順に処理する。`--lang fr --help` のような lang エラーと help の同時発生では help が勝つ (既存 `parse-args.ts` の help 最優先契約を維持)。後勝ち + 不正値の保持: `--lang fr --lang en` → `lang=en, error=invalid 'fr'` (`flag-parser.ts` の「不正値検出後に解析を停止」モデルと整合)。

**help 最優先の構造的保証**: `extractLang` は次トークンが `--` で始まる場合だけでなく、短形式の `-h` を含む `HELP_FLAGS` セットにヒットした場合も「値欠落」として扱い、その次トークンを **argv から消費せずに保持** する。これにより `--lang --help` / `--lang -h` / `--lang --clean` 等の組み合わせでも help / clean モード判定が `lang error reject` より構造的に先に通る。`-h` のような `--` prefix を持たない短形式が `invalid_value` として誤消費される退行は test で固定する (`preextract-lang.ts` の test)。

`--lang` は **CLI 自体の help / stderr 出力にのみ作用** し、生成 HTML の初期言語には影響を与えない (HTML 側は localStorage / navigator.language で独立に決定)。

### 14.5 翻訳キー命名規約と複数形分岐

12 名前空間 (`toolbar.*` / `comments.*` / `modal.*` / `toast.*` / `empty.*` / `page_nav.*` / `search.*` / `dialog.*` / `diagram.*` / `online.*` / `footnote.*` / `cli.*`) に整理 (総数約 195)。
...
HTML markup には `data-i18n="comments.empty"` 等の dataset 属性を持たせ、起動時に `applyI18nDataset(document)` が DOM を 1 回 walk して各属性を翻訳する。walk セレクタは `[data-i18n], [data-i18n-aria-label], [data-i18n-placeholder], [data-i18n-title], [data-i18n-data-tooltip]` の和、`#doc` 配下は `closest('#doc')` で除外する (アンカリングの textContent 計算 §6 を保護)。
...
- **4 関数パターン (state あり)**: `setup` / `show` (state 更新) / `clear` (state null) / `teardown` (全 subscription 解除 + state null + DOM 初期化) を持つ。表示中の文言が言語切替に追従するために state を保持する必要がある動的 UI に適用 (`source-display.ts` の現在表示中の URL、`paste-markdown-modal.ts` の現在のエラー key、`error-display.ts` の現在の renderer 等)
- **2 関数パターン (state なし)**: `setup` / `teardown` のみ。`subscribeLangChange` 内で module-local singleton (`state.pages` / `searchState` 等の app-state 直結) を都度読んで再描画する動的 UI に適用 (`page-navigation-render.ts` の `renderPageNavigation`、`search-dom.ts` の `updateCountDisplay`)。state を二重に持たないため `show` / `clear` 相当は不要
...
辞書 2 言語の raw size は約 6–7 KB (~195 entry × 平均 20 bytes × 2 言語)、helper (`i18n-core` / `i18n-browser`) は 1 KB 未満。対応言語数が 3 以上に増えた段階で「全言語を HTML に常時 inline」方式は配布物サイズを線形に押し上げるため、Shiki grammar / Mermaid runtime 等と同じく「必要な言語だけ後から fetch する」方式 (lazy load / 別ファイル分割) への移行を検討する。本実装は 2 言語のみのため inline + 設定モーダル内 select box を採用。
