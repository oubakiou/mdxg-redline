...
- **書き換え方法**: 配布者は次の 2 つから選べる。
  - **CLI 経由（推奨）**: `node dist/embed.mjs <input.md> <output.html>` で markdown を読み込み、`<script id="embedded-md">` の中身と `data-name` 属性を書き換えた HTML を生成する。`</script>` の自動エスケープと属性エスケープも CLI 側で適用される。実装は `src/embed.ts`（Node CLI ラッパー）+ `src/embed-core.ts`（pure ロジック）で、後者はブラウザ側からも再利用できる
...
ユーザーが選んだローカルディレクトリをブラウザが 2 秒間隔（`WS.POLL_MS`）でポーリングし、`review.md` の SHA-256 変化を検知すると自動でロード、`Submit review` で同じディレクトリに `feedback.json` を書き戻す方式。エージェント ↔ 人間のフィードバックループを「ファイルの読み書き」だけで成立させる中核経路。

- **想定ユースケース**: LLM エージェントが `review.md` を生成 → 人間がインラインコメント → エージェントが `feedback.json` を取り込み → 改訂版 `review.md` を再生成、という反復ループ。プロトコル詳細は §8
...
- **未送信作業の保護**: 現バージョンにコメントが付いている状態で `review.md` の内容が変わると、確認ダイアログでロード可否を問う。同じハッシュを一度拒否したら、別ハッシュに変わるまで再確認しない（§8 安全装置）
...
- **`npx` でブラウザ自動起動 (`file://` 直接オープン方式)**：起動 UX を `npx mdxg-redline` 一発に短縮する。バックエンド不要・依存最小の構成として `file://` 直接オープンを採用する。詳細計画は `docs/NPX_CLI_PLAN.md` を参照
  - **Phase 1（実装済み）**：埋め込みコアは `src/embed-core.ts`（pure）+ `src/embed.ts`（Node CLI）として実装され、`dist/embed.mjs` に SSR ビルドされる。`node dist/embed.mjs <input.md> <output.html>` で markdown を埋め込んだ HTML を生成できる
...
なし。`dist/review.html` をブラウザで開くだけ。配布者が markdown を埋め込みたい場合は `node dist/embed.mjs <input.md> <output.html>` を使う（§3 入力 2 参照）。
...
│   ├── embed-core.ts        embed CLI の pure ロジック (escape + rewrite)。
│   │                         Node / browser 両対応で、将来クライアントサイドからも再利用可能
│   ├── embed.ts             Node CLI (`dist/embed.mjs` のエントリ)。embed-core を呼んで
│   │                         <input.md> から <output.html> を生成する shebang 付きスクリプト
