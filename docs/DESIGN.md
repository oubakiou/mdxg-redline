...
- `cli/parse-args.ts`：CLI 引数パースと出力ファイル名サニタイズ
...
│   │   │                     review-request CLI とブラウザ側 boot.ts の双方から再利用
...
│   │   ├── review.ts        DOM エントリ、modal / menu / event wiring、loadFromMarkdown orchestrator
│   │   ├── boot.ts          起動順序（workspace handle 復元 / embedded）
│   │   ├── app-state.ts     state 単一の真の源 + Write feedback.json の dirty 追跡
│   │   ├── dom-utils.ts     qs / qsInput / uid / hashStr / toast
│   │   ├── mark-engine.ts   state.comments → <mark class="cmt"> の再適用エンジン
│   │   ├── doc-renderer.ts  markdown を HTML 化して #doc に流し込む
...
│       ├── parse-args.ts    引数パース + HELP_TEXT + sanitizeMdFileName
...
- 編集対象は `src/` 配下のみ（`review.html` / `styles/*.css` / `cli/*.ts` / `app/*.ts` / `core/*.ts`）。`dist/review.html` は `vp build` で都度上書きされるため手で直さない
