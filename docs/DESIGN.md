...
- **Content Security Policy**: 配布 HTML（`dist/standalone.html` / `dist/embed-template.html` および CLI が生成する `*-review.html`）すべてに `<meta http-equiv="Content-Security-Policy">` を埋め込み、二重保険で攻撃面を狭める。`file://` で開かれる前提のため HTTP ヘッダではなく meta で指定する。両 HTML は共通の `src/review.html` を入力に派生するため CSP も同一。内容は次のとおり：
  ...
  - `font-src data:` — KaTeX 数式フォント (20 woff2 family) を `url(data:font/woff2;base64,...)` で inline するため必須。`'self'` / `https:` は追加しない（外部フォント取得経路を持たない `data:` のみで完結、§12 §14 Math Rendering / `docs/mdxg-math-rendering.md` §5.g）。`font-src` ディレクティブを書かないと `default-src 'none'` に fallback して deny されるため、明示的に書く必要がある（CSP Level 3 仕様）
...
**本実装との差異**: リファレンス実装は §14 未対応。本実装は KaTeX を Mermaid (§15) と完全に対称な配布契約（standalone は default 全部入り / CLI 経路は `--math <auto|on|off>` で opt-in）で取り込み、`scanMath` ベースの renderer + paint 後 idle upgrade + `katex-error` class 判定の組合せで MDXG §14 [SHOULD] を満たす最短経路を採用。設計プラン詳細は [`docs/mdxg-math-rendering.md`](./mdxg-math-rendering.md) を参照。
...
3. `dist/katex/katex.mjs` / `dist/katex/katex.css` / `dist/katex/katex-fonts-extra.css` の 3 ファイルが揃っていれば読み込み、`<script id="embedded-katex" type="module">` / `<style id="embedded-katex-css">` / `<style id="embedded-katex-fonts-extra-css">` に inline する（standalone は `--math-fonts all` 相当固定、§12 §14 Math Rendering / `docs/mdxg-math-rendering.md` §5.k）。いずれかが ENOENT のときは stderr に警告を出して skip し、raw `$...$` plain text fallback のみが残る形で書き出す
