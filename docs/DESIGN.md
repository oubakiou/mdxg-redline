...
- [MUST] 検索の起動: ✓（`/` キー or toolbar 検索ボタン (`#btn-search`) で `openSearch()` を呼ぶ。`/` は `handleAffordanceKeys` 経由で `f` / `g` / `h` / `?` と同じ isEditableTarget / hasNoModifier ガードを通す。Cmd/Ctrl+F は触らない設計で、ユーザーは「ブラウザ標準のサイト検索」と「ドキュメント検索」を使い分けられる）
...
- **`g` キーで doc-pane フォーカス**: `f` で TOC に飛んだ後、本文を ↑↓ / PgUp/PgDn でスクロールしたい時に doc-pane へ focus を戻すための逆経路。`event.code === 'KeyG'` + 修飾キー無しを検知し、`<section class="doc-pane">` (`tabindex="-1"`) に `focus()` する。`f` と同じ affordance スキーマ (`isEditableTarget()` ガード / `hasNoModifier` / `event.repeat` 抑止) を共有。focus 中は `.doc-pane-keyhints` (`<kbd>↑↓</kbd><kbd>PgUp/PgDn</kbd><kbd>f</kbd>nav`) を doc-pane 右上に ephemeral 表示し、`.page-nav-keyhints` と対称な `:has(:focus-visible)` / `:focus-visible` ルールで keyboard focus 起因のときだけ visible にする (マウスクリック起因の `:focus-within` チラつきを避けるのも同じ理由)。keyhints は `height:0` の sticky anchor (`.doc-pane-keyhints-anchor`) で包み、後続の `.virtual-page` を押し下げずにスクロール時の縦位置を上端に固定する
- **`h` キーで comments panel フォーカス**: 右サイドのコメントパネルをキーボードで操作する経路。`event.code === 'KeyH'` + 修飾キー無しを検知し、`focusActiveOrFirstCommentCard()` で「`.cmt-card.active` → 最初の `.cmt-card` → `<aside class="comments">` (`tabindex="-1"`) 自身」の優先順で focus を移す (TOC の `focusNavigatedLink` と対称、空状態時の hint へのアクセスを保つため最後段で aside 自身に fall back)。各 `.cmt-card` は `tabIndex = 0` で flat tab order に乗り、`wireCommentsKeyboardNav` が `aside.comments` 上の delegated keydown listener で ↑↓ / Home / End によるカード巡回を捌く (page-navigation.ts の `onPageNavKeyDown` と同じ flat 配列巡回パターン)。Enter は cmt-card 個別 listener が捌き、既存の `focusCommentCard` を呼んで該当 `mark.cmt` に scroll + active 表示する (`<div>` は native click on Enter を持たないため明示的に handle)。`f` / `g` と同じ affordance スキーマを共有し、`.comments-keyhints` を `.doc-pane-keyhints` と同じ anchor (`height:0` / `position:sticky`) パターンで panel 右上に ephemeral 表示する
- **`?` キーで help modal**: `Shift+/` で発火する `event.key === '?'` を検知し `toggleHelpModal()` を呼ぶ。`f` / `g` / `h` と共通の `isEditableTarget()` ガード経由で文字入力を妨げない。modal HTML は `src/review.html` に静的に置き、`app/help-modal.ts` が `open` クラスの toggle と Close ボタン / バックドロップクリックの wiring を担当する。閉じる経路: Close ボタン / バックドロップクリック / グローバル Esc (既存 `handleEscapeKey` に `closeHelpModal()` を統合)
