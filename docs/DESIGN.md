...
ビルドパイプラインは開発者ローカルでのみ動作。配布物 (`dist/standalone.html` / `dist/embed-template.html` / `dist/hosting/index.html` / `dist/review-request.mjs` / `dist/shiki-langs/*.json` / `dist/hosting/canonical/shiki-langs/*.json` / `dist/hosting/fingerprinted/shiki-langs/*.<hash>.json` / `dist/mermaid.mjs` / `dist/katex/katex.mjs` / `dist/katex/katex.css` / `dist/katex/katex-fonts-extra.css` および Cloudflare Pages 用 `dist/hosting/_headers`) はすべてビルド成果物としてリポジトリにコミットされており、エンドユーザー環境にはツールチェーンを持ち込まない。 `dist/hosting/` は Pages の Build output directory として指定する配信 subset (online edition の runtime fetch 用 canonical / fingerprinted を含む、`docs/feature-online-runtime-assets.md` §5.f / §5.i / Step 4)、 standalone / CLI 経路 (dist 直下) では参照しない。
...
ビルドの出口は 4 つ。エンドユーザーが直接開く配布物 `dist/standalone.html`、review-request CLI が rewrite テンプレートとして読み込む `dist/embed-template.html`、ホスティング配信用の `dist/hosting/index.html`（URL fetch viewer、§3 入力 3）、配布者向け CLI ツール `dist/review-request.mjs`。加えてホスティング先 (Cloudflare Pages 等) 用の静的設定ファイル `dist/hosting/_headers` が build 時に emit される (Cloudflare Pages の Build output directory に `dist/hosting` を指定する設計、`docs/feature-online-runtime-assets.md` Step 4)。
...
| `dist/hosting/index.html`                                   | C1   | ホスティング配信用の URL fetch viewer（§3 入力 3）。Phase A.2 で grammar inline を空 `{}` に、`<script id="online-asset-manifest">` を `<head>` に注入。Mermaid / KaTeX は Phase B / C で剥がす予定 (`docs/feature-online-runtime-assets.md` 参照)                                             | ~4.2 MB / gzip ~1.3 MB |
...
| `dist/hosting/canonical/shiki-langs/<lang>.json`            | C1   | online edition の runtime fetch 用 (hash 無し)。新 deploy 過渡期の 404 retry 先 (`docs/feature-online-runtime-assets.md` §5.i の 3 段 fail-safe)                                                                                                                                               | (言語ごとに分割)       |
...
**grammar の多重生成を許容する根拠**: build 後の `dist/` 配下では grammar JSON が `dist/shiki-langs/<lang>.json` (CLI 配布物、dist 直下)、`dist/hosting/canonical/shiki-langs/<lang>.json` (online edition canonical fallback、 hosting 配下)、`dist/hosting/fingerprinted/shiki-langs/<lang>.<hash>.json` (online edition immutable cache 対象、 hosting 配下)、および `dist/standalone.html` に inline された全 grammar の **4 系統** に存在する。DRY ではないが、すべて同じ `emitGrammarJsonFiles()` の単一呼び出しから生成され、毎 build 冒頭で 3 ディレクトリを clean してから書き出すため drift は構造的に起こらない。 dist 直下 (CLI / Releases 配布用) と `dist/hosting/` (Pages 配信用 subset) の境界が物理的に明確 (Pages の Build output directory に `dist/hosting` を指定する設計、`docs/feature-online-runtime-assets.md` Step 4)。 standalone は `file://` での fetch path 制約と CSP `connect-src 'none'` を保つため inline 維持、online edition だけが `connect-src 'self'` を許可して runtime fetch する分岐構造（`docs/feature-online-runtime-assets.md`）。symlink 案は Windows / npm publish 互換性が落ちるため不採用。
...
| 出力分岐                    | mdxg-split-outputs     | closeBundle 内で逐次 await: ① `emitGrammarJsonFiles()` で grammar JSON を 3 系統 (CLI / hosting/canonical / hosting/fingerprinted) に emit + manifest payload を返す。② 中間出力 `dist/review.html` を読み、③ `<script id="embedded-shiki-langs">` に全 grammar を inline + `dist/mermaid.mjs` / `dist/katex/*` が揃っていれば各 embed ブロックに inline (不在時は警告 skip) して `dist/standalone.html` を書き出す。④ standalone から `buildOnlineHtml(standaloneHtml, { allowlist, manifest })` で `dist/hosting/index.html` を派生 (Phase A.2 の 5 mutation: `data-mdxg-online`、CSP `'self'` prepend、`<script id="online-allowlist">`、`<script id="online-asset-manifest">`、空 `<script id="embedded-shiki-langs">{}`)。⑤ Cloudflare Pages 用 `dist/hosting/_headers` (CSP + path 単位 Cache-Control) を emit (`_redirects` は不要、 Pages が `/` で自動的に index.html を返す)。⑥ 中間出力 `dist/review.html` を `dist/embed-template.html` にリネーム |
