...
- **再選択時の挙動**: 読み込むたびに `state.comments` は空に初期化される。過去ラウンドのコメントを引き継ぎたい場合は、エージェント側で前ラウンドの `*-feedback.json` を `<script id="embedded-feedback">` として同梱した HTML（review-request CLI 経路）で配布するか、URL ハッシュ経路に渡す（IndexedDB からの復元は origin 安定性が保証されないため廃止 / §7）
...
過去には `doc:<docHash>` キーでドキュメント本文＋コメントを永続化し「タブを閉じて開き直すと続きから」を提供していたが、review-request CLI の HTTP サーバーモードはポートごとに origin が分かれ（デフォルトポート方式でも初回起動／衝突 fallback 時には origin が動く）、`file://` モードも Firefox / Safari ではファイル単位で origin が分離するため、復元が保証できない。さらにラウンドごとに `docHash` が変わる前提と相まって `doc:<docHash>` レコードは「使い捨ての履歴」として溜まるだけになるため、この用途は廃止した（タブを跨いだコメント保持は `Submit review` で feedback.json を書き出す運用に寄せる）。

### b. File System Access API（ワークスペースモード）

ファイル監視プロトコル用。`showDirectoryPicker()` が返す `FileSystemDirectoryHandle` 自体を IndexedDB の `workspace-handle` に保存。再読み込み時：
...
ワークスペース連携・review-request CLI 配布フローを通じて、すべてのレビューパッケージはこの命名規約に従う：
...
ページロード時に以下の優先順チェーンを実行。ワークスペース復元は先に試すが、`*-review.md` の読み込みはポーリングで遅延することがあるため、それだけでは後続の入力経路を止めない。埋め込み・URL hash・既存 state のいずれかで本文が確定した時点で終了する。
...
過去には「Store から最終更新ドキュメントを復元」する起動ステップを持っていたが、§7 に書いたとおり origin 安定性が保証されないため廃止。タブを跨いだ続きの作業は `Submit review` での feedback.json 書き出しに寄せる。
...
- IndexedDB の用途は「ワークスペース ディレクトリハンドルの保存」のみ（§7）。コメント本体や本文は IndexedDB に書かない。ハンドルは origin に紐づくため、別のパスやオリジンで開けば再ピッカーが必要になる
...
- **Watch folder を `Write feedback.json` 1 ボタンに統合・縮小する**：review-request CLI の導入で、Watch folder が担っていた責務のうち (A)「`*-review.md` を 2 秒ポーリングで自動検知・ロード」は機能的に重複した（エージェントは CLI を呼ぶ方が早くて素直、devcontainer/Codespaces にも強い）。残るのは (B)「`feedback.json` を File System Access API 経由でフォルダに書き出す」だけになり、これは厳密には「監視」ではなく「書き出し先フォルダの保持」なので、Watch という概念モデル自体が不要になる。さらに、書き出し先のフォルダ選択も「`showDirectoryPicker` をアクション実行時にオンデマンドで呼ぶ」運用にすれば、`Output folder` ボタンも独立 UI として持つ必要がない
  - **統合後の UI**：`Watch folder` / `Output folder` / `Reconnect · フォルダ名` の 3 ボタンを廃止し、プライマリ位置に GitHub の `Merge pull request` 風の split button `[Write feedback.json] [▾]` を 1 つ置く（既存 §3 出力 3 `Submit review` のリネーム＋常時表示化）。本体ボタンの状態別挙動：
    - 初回押下（ハンドル未保存）：`showDirectoryPicker()` を呼んでフォルダを選ばせ、選択されたフォルダに即書き出し → ハンドルを IndexedDB に永続化
    - 2 回目以降（ハンドルあり、権限 OK）：保存済みフォルダにそのまま書き出し
    - 権限失効時（`queryPermission` が `prompt` 相当）：内部で `requestPermission()` を呼ぶ。通れば書き出し、拒否されれば picker にフォールバック
    - ハンドル無効化（フォルダ削除等）：catch して picker を再呼び出し
  - **前提条件：review-request CLI の HTTP モードでデフォルトポートを採用**：現状の HTTP サーバーモードはランダムポートのため、起動するたびに origin (`http://localhost:<port>`) が変わり IndexedDB の `workspace-handle` 永続化が機能しない。これを「デフォルトポート（例：`51729`、Dynamic/Ephemeral 範囲の 5 桁高ポート）から起動を試み、環境変数 `MDXG_REDLINE_PORT` で上書き可能。デフォルト（or 環境変数指定）が使用中ならランダムポートへ fallback し、stderr に『デフォルトポート X が使用中のため Y を使用しています。IndexedDB のサイレント復元は今回効かない可能性があります』と警告」の方式に変える。これにより初回起動から origin が安定し、サイレント書き出し UX が HTTP モードでも成立する。`.last-http-port` のような状態ファイルが不要で、VS Code Remote / Codespaces の `forwardPorts` 設定に固定値を書ける副次的なメリットもある
  - **縮小範囲（コード）**：`*-review.md` のポーリング、自動ロード、未送信作業の保護ダイアログ、§8 のライフサイクル 2-4 / 安全装置の大半、`WS.INPUT_PATTERN` / `WS.POLL_MS` 定数、`Output folder` / `Reconnect` ボタン関連の DOM・イベント配線を削除。`workspace.ts` の主要処理は「ディレクトリハンドル取得・永続化・権限再要求・feedback.json 書き出し」だけに縮約され、エントリ関数は `writeFeedbackJson()` 1 つに近づく（現行 `workspace.ts` ~200 行規模 → ~60 行規模を見込む）
  - **書き出し先の発見性**：書き出し後の toast に「`<folderName>/<fileName>.json` に書き出しました」とフルパス相当を表示すれば、ユーザーが書き出し先を確認する導線として十分（既存 toast 機構で実装可能）。`docHash` が変わって別 MD を扱っている場合も toast のフォルダ名で気付ける前提
  - **フォルダ切り替えの導線**：split button の `[▾]` 部分をクリックすると drop-down が開き、`Change output folder…` 項目から picker を再呼び出しできる。HIG として正統な split button パターンで、`Comments ▾` メニューに混ぜないことで「コメント操作」と「書き出し先操作」の関心を分離する。drop-down 内容は当初 `Change output folder…` 1 項目だけで開始し、必要に応じて `Show in finder` 等を追加
  - **影響範囲**：DESIGN.md §3 入力 4 / §3 出力 3 / §7 / §8 / §9 / §10 / §12 の書き換え、`workspace.ts` / `dialog.ts` / `boot.ts` / `toolbar.ts`（split button 化）のリファクタ、ファイル命名規約 (`*-review.md`) は CLI 経路ではエージェント側の責務に移る（ブラウザは検知しなくなる）
  - **代替シナリオ**：Watch がカバーしていた (C)「同一タブで複数ラウンドを自動で回す」体験は失われる。複数ラウンドはエージェントが review-request CLI を都度呼んで新タブで開くフローになる。デフォルトポート方式と組み合わせれば、同一ポート＝同一 origin で再開されるため `workspace-handle` は維持され、新タブでも即座に書き出し先フォルダが復元される。これを許容できるかが採用判断の鍵
...
│   ├── boot.ts              起動順序（workspace / embedded / URL hash）
