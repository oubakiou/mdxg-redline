...
- **書き換え方法**: 配布者は `dist/review.html` をテキストエディタで開き、当該ブロックの中身を差し替える運用を想定。CLI でこの工程を自動化する案は §13 の `npx` 拡張候補を参照
...
- **必要 API**: `showDirectoryPicker` と `FileSystemDirectoryHandle`。**Chromium 系のみ**（Chrome / Edge / Arc / Brave / Opera）。Safari / Firefox では `Watch folder` ボタンが無効化される
