...
- **モバイル UI の残課題**：≤768px の主要 chrome（上端固定ヘッダ + footer バー + 左右 drawer + search-bar、navigation 経路の drawer 自動 close、Edit modal trigger の drawer 自動 close、focus 復元契約、breakpoint 切替時の focus 退避）に加え、範囲選択時の floater を footer 直上のフル幅固定バー化してネイティブ選択メニューとの重なりを回避する対応も実装済み（§4 モバイルレイアウト / §10、`docs/archive/feature-mobile-layout.archive.md` §5.t）。次フェーズ候補として以下が残る：
