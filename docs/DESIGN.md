...
| LLM がコンテンツ事前読み込みで起動できること                                        | 複数の注入経路（埋め込み・URL ハッシュ・ファイル選択）。URL ハッシュは Base64URL を推奨                       |
...
markdown をページに入れる方法が 3 つ、フィードバックを取り出す方法が 3 つあり、自由に組み合わせ可能。

### 入力

markdown を画面に乗せる経路は 3 つ。それぞれ「誰が markdown を持ち込むか」が異なる。起動時の優先順位は §9 を参照。
...
- **サイズ制限**: ブラウザによって URL 長の上限が異なり、実用上は 8 KB 程度が安全圏。それを超える文書では埋め込み方式に切り替える
...
ページロード時に以下の優先順チェーンを実行。出力先ハンドル復元は副作用が無く後段を阻害しないため、本文ロードのパスとは独立に走らせる。埋め込み・URL hash のいずれかで本文が確定した時点で終了する。
...
2. URL ハッシュ (#md=<base64url>&name=<optional>)
   └─ デコードしてロード

3. どれも該当しなければ空状態のまま `Open file` を待つ
```

過去には「IDB から最終更新ドキュメント本文も復元」「`*-review.md` ポーリング」の起動ステップを持っていたが、§7 に書いたとおり origin 安定性が保証されない / review-request CLI で配布責務を移したため廃止。タブを跨いだ続きの作業は `Write feedback.json` での書き出しと、エージェント側 (CLI) の HTML 再生成に寄せる。
...
Safari と Firefox はファイル選択・埋め込み・URL ハッシュ・コピー / エクスポートのフローは使えるが、**Write feedback.json は利用不可**（Export as JSON / Copy as JSON で代替）。
...
│   ├── boot.ts              起動順序（workspace handle 復元 / embedded / URL hash）
