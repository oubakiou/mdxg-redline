...
- `--shiki-langs <value>` で `<script id="embedded-shiki-langs">` に注入する Shiki grammar を選ぶ。`auto`（既定）は `marked.lexer` で markdown のフェンス言語を抽出し必要分だけ注入、`all` は Shiki bundled 全言語（約 235）全部、`none` は注入しない（全コードブロックを plain text fallback）、`<csv>` (`ts,js,py` 等) はエイリアス正規化を経て指定された正規名集合だけを注入。詳細は §12 §2 Code Block Rendering 行
- `--mermaid <value>` で `<script id="embedded-mermaid" type="module">` への Mermaid runtime 注入を制御する。`auto`（既定）は `scanMermaidFences` で markdown 内の ` ```mermaid ` ブロック数を数え、1 件以上のときだけ注入（配布物サイズが内容と連動）、`on` は件数に関係なく必ず注入、`off` は注入しない（` ```mermaid ` ブロックは Shiki ハイライト fallback として表示、MDXG §15 [MUST]）。注入時の配布物サイズ増分は約 +700 KB gzipped。auto で検出した場合は stderr に「Detected N mermaid block(s).」を 1 回報告。詳細は §12 §15 Diagram Rendering 行
- `--math <value>` で `<script id="embedded-katex" type="module">` / `<style id="embedded-katex-css">` への KaTeX runtime 注入を制御する。`auto`（既定）は `countMath` で markdown 内の `$...$` / `$$...$$` 件数を数え、1 件以上のときだけ注入（配布物サイズが内容と連動）、`on` は件数に関係なく必ず注入、`off` は注入しない（`$...$` / `$$...$$` は raw な markdown 文法のまま plain text として表示、MDXG §14 [MUST]）。auto で検出した場合は stderr に「Detected N math expression(s). Embedding KaTeX runtime (fonts=<minimal|all>, +~<sz> KB gzipped).」を 1 回報告。詳細は §12 §14 Math Rendering 行
- `--math-fonts <value>` で KaTeX 注入時のフォント family 範囲を制御する。`minimal`（既定）は Main / AMS / Math / Size1〜4 の 9 family のみ data URI で inline（gzip +~240 KB）、`all` は Caligraphic / Fraktur / Script / SansSerif / Typewriter の 11 family も追加 inline（gzip +~340 KB）。`\mathcal` / `\mathfrak` / `\mathscr` / SansSerif / Typewriter が必要な文書のみ `all` を指定する。`--math off` のときは `--math-fonts` の値は黙って ignore（注入経路自体が走らないため、Shiki の `--shiki-langs none` × csv と同じ振る舞い）。詳細は §12 §14 Math Rendering 行
...
- `--markdown-css <path>` で本文プレビュー用 CSS (`src/styles/markdown.css` のデフォルト) をユーザー指定ファイルに差し替える。差し替え対象は配布 HTML 内の `<style id="markdown-css">` ブロックのみで、レイアウト / chrome 用の review.css は不変。スコープは `#doc` 配下が対象である規約とし、`body` / `.app-header` 等の chrome セレクタを書いた場合の review.css との競合は CSS 優先度に従う。書き込み時に literal `</style>` を `<\/style>` に escape し、CSS コメントや `content:` 値に閉じタグ文字列が混入してもパースが壊れない。`<style id="markdown-css">` は `src/review.html` に置かれており、build 時の `markdownCssInlinePlugin` がデフォルト markdown.css を inline、CLI 経路ではその中身を差し替える経路で `embed-template.html` / `standalone.html` 双方で `<link>` 統合の `<style>` とは別ブロックに分離されている
...
整理手段として review-request CLI に `--clean <dir>` サブコマンドを用意してある（`src/cli/clean.ts`）。`<dir>` 直下の `*-<docHash>-review.html` / `*-<docHash>-feedback.json` を命名規約の正規表現 `^(.+)-([0-9a-f]{16})-(review\.html|feedback\.json)$` で抽出し、まとめて削除する。サブディレクトリは触らない（`-maxdepth 1` 相当）。
...
- **Mermaid runtime の信頼境界**: ` ```mermaid ` ブロックを SVG に upgrade する Mermaid.js は `mermaid.initialize({ securityLevel: 'strict' })` で固定起動し、`<foreignObject>` / 任意 HTML 文字列の挿入を sanitize する。配布 HTML に inline される runtime 本体は `<script id="embedded-mermaid" type="module">` ブロックで、`script-src 'self' 'unsafe-inline'` の既存許可範囲内で動作するため CSP の緩和は不要。CLI 注入時に runtime 内の literal `</script>` を `<\/script>` に escape し、Mermaid version up でエラーメッセージ等に `</script>` が混入しても build を fail させない。bridge は `globalThis.__mdxgMermaid` の 1 識別子のみ (`__` prefix で本実装スコープを明示) で、他コードとの衝突を構造的に避ける（詳細は §12 §15 Diagram Rendering 行 / docs/mdxg-diagram-rendering.archive.md）
- **KaTeX runtime の信頼境界**: `$...$` / `$$...$$` を upgrade する KaTeX は `katex.renderToString(src, { trust: false, strict: 'warn', throwOnError: false, errorColor: 'inherit' })` で固定呼び出しする。`trust: false` で `\href` / `\url` / `\includegraphics` / `\htmlClass` 等の外部リソース系コマンドが `<mtext>\href</mtext>` のような escape された best-effort 描画になり、`<a href>` / 任意 HTML / class 適用は一切出力されない（Step 1 PoC 実機確認、`docs/mdxg-math-rendering.archive.md` §5.f）。`throwOnError: false` で文法エラーは `<span class="katex-error">` を返し、ブラウザ側 `src/app/katex.ts` がそれを検出して `data-math-failed="1"` 付与 + toast 集約通知に倒す。配布 HTML に inline される runtime 本体は `<script id="embedded-katex" type="module">` ブロックで、`script-src 'self' 'unsafe-inline'` の既存許可範囲内で動作する。CLI / build plugin 双方が注入時に literal `</script>` を `<\/script>` に escape し、`<style id="embedded-katex-css">` / `<style id="embedded-katex-fonts-extra-css">` への CSS 注入時にも `</style>` を `<\/style>` に escape する。bridge は `globalThis.__mdxgKatex` の 1 識別子のみで、Mermaid と同じ `__` prefix 規約に従う（詳細は §12 §14 Math Rendering 行 / docs/mdxg-math-rendering.archive.md）
...
- **Content Security Policy**: 配布 HTML（`dist/standalone.html` / `dist/embed-template.html` および CLI が生成する `*-review.html`）すべてに `<meta http-equiv="Content-Security-Policy">` を埋め込み、二重保険で攻撃面を狭める。`file://` で開かれる前提のため HTTP ヘッダではなく meta で指定する。両 HTML は共通の `src/review.html` を入力に派生するため CSP も同一。内容は次のとおり：
  ...
  - `font-src data:` — KaTeX 数式フォント (20 woff2 family) を `url(data:font/woff2;base64,...)` で inline するため必須。`'self'` / `https:` は追加しない（外部フォント取得経路を持たない `data:` のみで完結、§12 §14 Math Rendering / `docs/mdxg-math-rendering.archive.md` §5.g）。`font-src` ディレクティブを書かないと `default-src 'none'` に fallback して deny されるため、明示的に書く必要がある（CSP Level 3 仕様）
...
| Extensions         | [§15 Diagram Rendering](./mdxg/05-extensions.md#15-diagram-renderingダイアグラム描画)                 | 部分 | `mermaid` のみ対応 (Mermaid.js + `securityLevel:'strict'`、theme 連動再描画、クリックで拡大モーダル)。`plantuml` / `d2` / `graphviz` は未対応      |
| Extensions         | [§16 Footnotes](./mdxg/05-extensions.md#16-footnotes脚注)                                             | 準拠 | marked-footnote 拡張 + 文書末 synthetic page + `core/footnotes.ts` の post-processing で未参照定義を救済 (詳細は `docs/mdxg-footnotes.archive.md`) |
...
**footnotes synthetic page**: MDXG §16 Footnotes 対応で、markdown に `[^id]: ...` 定義が ≥1 個あると `appendFootnotesPage` が末尾に `slug: 'footnotes'` / `title: 'Footnotes'` / `depth: 1` の synthetic page を追加する (`docs/mdxg-footnotes.archive.md` §3.2 / §5.c)。TOC には他 page と同じく `a.page-nav-link` として現れ、クリックで `navigateToTarget` 経由で `<section class="virtual-page" data-page-slug="footnotes">` (実装上は `resolveUniqueSlug` で本物の H1 "Footnotes" との衝突を `-2` suffix で回避) にジャンプする。`#footnote-<id>` / `#footnote-ref-<id>` の deep link は `app/review.ts` の `handleFootnoteHash` が `data-page-index` から逆引きして synthetic page を active 化する別経路 (page slug hash と構造的に分離) で処理する。
...
- **Mermaid upgrade 済みダイアグラムは検索対象外**: `selection.ts` の `textSegments` が `[data-mermaid-applied]` (`<pre hidden>`) / `[data-mermaid-svg]` (兄弟 `<svg>`) 配下を skip するため、upgrade 後のダイアグラム内文字列は `collectSearchMatches` の戻り値に含まれず `#search-count` の件数にも反映されない。SVG 内 textContent (ノードラベル / arrow ラベル) は構造が予測不能でヒット位置の `scrollIntoView` 着地点を一貫させづらいことが採用根拠 (§12 §15 Diagram Rendering 行 / `docs/mdxg-diagram-rendering.archive.md` §4 Step 6 案 A)。未 upgrade (`data-mermaid="1"` のみ) の `<pre>` は通常どおり拾われ、Shiki ハイライト fallback 時の検索対象として残る
...
- [SHOULD] `$...$` インライン数式: ✓（`src/core/markdown.ts` の `renderer.text` が `scanMath` で text run を走査し、`<span data-math="inline" data-math-source="…">$x$</span>` を出力。`src/app/katex.ts` が paint 後 idle で `katex.renderToString` を呼び KaTeX HTML に置換）
...
- **配布契約**:
  ...
  - **`dist/katex/katex.mjs`**: KaTeX 0.16.47 を Vite `build.minify: 'esbuild'` で 1 ESM 化（raw 259 KB / gzip 77 KB）。`src/katex-entry.ts` の bridge コード (`globalThis.__mdxgKatex` + `mdxg:katex-ready` イベント) が末尾に焼き込まれる
...
- **bridge global**: `<script id="embedded-katex" type="module">` 末尾で `globalThis.__mdxgKatex = katex` をセットし `mdxg:katex-ready` イベントを発火。ブラウザ側 `src/app/katex.ts` は paint 後 `requestIdleCallback` で global を読み出し、未到達なら最大 2s イベントを待つ（Mermaid と同じ規約）
...
- **§6 / §10 との両立**: `selection.ts` の `textSegments` が `[data-math]` 要素を hasAttribute で skip するため、upgrade 前の `$x$` (3 chars) と upgrade 後の KaTeX MathML+HTML (数十 chars) で textContent が大きく変化しても周辺 text node のオフセットが完全に不変（§6 アンカリング維持）。代償として §1 で宣言した「数式そのものへのコメント付与は対応外」、§10 Search の LaTeX ソース検索は将来拡張
...
**本実装との差異**: リファレンス実装は §14 未対応。本実装は KaTeX を Mermaid (§15) と完全に対称な配布契約（standalone は default 全部入り / CLI 経路は `--math <auto|on|off>` で opt-in）で取り込み、`scanMath` ベースの renderer + paint 後 idle upgrade + `katex-error` class 判定の組合せで MDXG §14 [SHOULD] を満たす最短経路を採用。設計プラン詳細は [`docs/mdxg-math-rendering.archive.md`](./mdxg-math-rendering.archive.md) を参照。

#### §15 Diagram Rendering（部分）

- [SHOULD] ` ```mermaid ` ブロックのサポート: ✓（`src/app/mermaid.ts` が paint 後 idle で各 `<pre data-mermaid="1">` を順次 SVG に upgrade。`<pre>` を `hidden` で残して sibling `<svg>` を挿入する構造で、§6 アンカリング / §10 Search は upgrade 後の DOM 上で案 A の skip により壊れない）
- [MAY] `plantuml` / `d2` / `graphviz` 等: 未対応（本実装スコープ外。各言語の描画エンジンが JVM / Go バイナリ依存で単一 HTML 配布物に inline できないため）
...
- **bridge global**: bundle 出力末尾で `globalThis.__mdxgMermaid = mermaid; document.dispatchEvent(new Event('mdxg:mermaid-ready'))` が走る。ブラウザ側 `src/app/mermaid.ts` は paint 後 `requestIdleCallback` (fallback `setTimeout(0)`) で global を読み出し、未到達なら最大 2s イベントを待つ
...
- **§6 / §10 との両立**: `selection.ts` の `textSegments` が `[data-mermaid-applied]` / `[data-mermaid-svg]` 子孫を skip するため、cmt mark の貼付経路と `collectSearchMatches` の両方が構造的に対象外になる (`.code-copy-btn` / `.code-lang-label` skip と同じパターン)。未 upgrade (data-mermaid="1" のみ) の `<pre>` は通常どおり拾われ、Shiki ハイライト fallback 時の検索 / コメント対象として残る
- **クリックで拡大モーダル**: 各 SVG に click / Enter / Space handler を wire し、`#mermaid-modal-backdrop` に SVG の `outerHTML` を複製挿入して表示する。modal は `98vw × 98vh` で SVG を flex 中央配置。inline `cursor: zoom-in` はクローン後に JS で剥がしてモーダル内ではデフォルトカーソルに戻す。Esc / 背景クリック / Close ボタンで閉じる経路は `help-modal.ts` と同じパターン。selection 中 / SVG 内 `<a>` (Mermaid `click` directive) クリックは open に転送しない
...
**参考**: Step 1〜8 の段階的実装手順、却下案 (B 案 `<pre>` を SVG で置換 / C 案 `<details>` 折りたたみ / D 案 `queueMicrotask` 即時 schedule など)、Mermaid 1 言語に絞った理由と将来 plantuml / d2 / graphviz 追加時の方針、theme トグル時の再描画フロー詳細などは [`docs/mdxg-diagram-rendering.archive.md`](./mdxg-diagram-rendering.archive.md) を参照。
...
- **DOM テストの追加**：`vite.config.ts` の `test.environment: 'happy-dom'` 採用後、`src/app/` および `src/core/` の DOM 依存ロジックへ regression test を順次追加していく方針。パイロットとして `src/app/mark-engine.ts` の `applyMarksForBlock` に 3 件追加済み（commit `93805da`）。次に追加すべき候補は調査の上で以下の優先度に整理してある（happy-dom 20 の API カバレッジ・既存 in-source test の重複・DESIGN.md §6 §10 §13 §15 §16 との結び付きで評価）：

  **Tier 0（既存 TODO 債務の回収、最優先）**
  - `src/core/footnotes.ts` の `extractFootnoteSection` / `renderOrphanFootnoteItems`：当該ファイル内に「別途 happy-dom 導入時にテストを足す」旨の明示 TODO コメントが残っており（`composeOrphanItemInnerHtml` の pure 部分のみ既テスト）、happy-dom 採用済みの今が回収タイミング。MDXG §16 [準拠] の決め手である「marked-footnote が落とす未参照定義の救済」経路で、複数段落 body の入れ子崩壊回避と組み合わさる。テストケース想定：(a) `extractFootnoteSection` が `<section data-footnotes>` を取り出す / null 戻り、(b) `renderOrphanFootnoteItems` が既存 `<ol>` に orphan li を append、(c) `<ol>` 不在時の `ensureOrphanOl` による合成、(d) section ごと欠落時の `createSyntheticFootnotesSection` 経路、(e) backref `<a>` の `href` / `aria-label` が pure 側（`composeOrphanItemInnerHtml`）と一致、(f) **orphan 定義複数件の文書順保持**、(g) **既存 footnotes section との merge 経路で重複・並び順崩れが起きない**

  **Tier 1（即座に追加推奨、純度高・happy-dom API 問題なし）**
  - `src/app/selection.ts` の `textSegments`：§6 アンカリングの中核。skip ルール（`SKIP_TEXT_SEGMENT_ATTR_NAMES` = `data-math` / `data-footnote-ref` / `data-footnote-backref`、`SKIP_TEXT_SEGMENT_CLASSES` = `sr-only` / `.code-copy-btn` / `.code-lang-label` 等）が継続的に増えており、upgrade 前後で textContent が変化しないという invariant が複数回 regression 源。テストケース想定：(a) plain text の平坦化、(b) skip 属性配下の除外、(c) skip クラス配下の除外、(d) Mermaid upgrade 済み (`data-mermaid-applied`) の sibling SVG 除外
  - `src/app/code-copy-wrap.ts` の `injectCopyButtons`：順序依存（`cacheBlockOriginalHTML` → `injectCopyButtons` → `applyMarksForBlock`）が DESIGN.md §6 / §12 §2 で繰返し言及。idempotent 性も検証価値あり。テストケース想定：(a) `<pre>` を `<div class="code-block-wrap">` で wrap、(b) Copy button を wrap 内に append、(c) `data-lang` から `<span class="code-lang-label">` を挿入、(d) 既 wrap 済み `<pre>` は no-op
  - `src/app/comments.ts` の active 双方向同期（`mark.cmt` ↔ `.cmt-card`）：本文 `<mark class="cmt">` クリックで対応する `.cmt-card` が active になり、逆も成立する経路。`commentCardHTML` 自体の escape は既テスト済みだが、active 反映のクラス付け替え・スクロール連携は未テスト。テストケース想定：(a) mark クリックで対応 cmt-card に `.active` 付与・他は剥がす、(b) cmt-card クリックで対応 mark に視覚 active 反映、(c) 削除ボタンの event 伝播抑制（cmt-card クリック経路を発火させない）、(d) 対応する mark が DOM に居ない（fail-soft）ケース

  **Tier 2（mark-engine と対称構造でコピペコスト低）**
  - `src/app/search.ts` の `wrapMatchInBlock` / `applyMatchesForBlock`：§10 Search 中核。startOffset 降順 wrap の不変条件は mark-engine と同パターン。`<mark class="cmt">` と `<mark class="search-hl">` の共存（同 text node 内 nest）の検査も同時に取れる
  - `src/app/doc-renderer.ts` の `innerHTMLForOriginalCache`：§6 アンカリング `blockOriginalHTML` 焼き込み除外の核。dynamic mark（`mark.cmt` / `mark.search-hl`）と `.code-block-wrap` を unwrap してから採取する不変条件は、Shiki / Mermaid / KaTeX upgrade 後の `refreshBlockOriginalHTML` 経路でも参照される。テストケース想定：(a) `mark.cmt` unwrap、(b) `mark.search-hl` unwrap、(c) ネスト mark（cmt 内 search-hl 等）の正規化、(d) `.code-block-wrap` の剥離で `<pre>` が裸に戻る、(e) Shiki span 入り innerHTML は触らず保持

  **Tier 3（contract 部分のみ、描画品質は対象外）**
  - `src/app/mermaid.ts` / `src/app/katex.ts` の upgrade フロー：本物の Mermaid / KaTeX runtime を inline できない以上、SVG / MathML の描画品質はテストできない。**契約 contract 層のみに限定**してテストする：(a) **idempotency**：同一 block 二重 upgrade で no-op（`data-mermaid-applied="1"` / `data-math-applied="1"` の再入抑止）、(b) **fail-soft**：fake runtime が throw → `data-mermaid-failed="1"` / `data-math-failed="1"` フラグ付与 + 集約 toast 通知が 1 回だけ、(c) **selection 退避**：`getSelection().toString().length > 0` で upgrade skip、空に戻ったら再試行、(d) **blockOriginalHTML 更新**：upgrade 後に `refreshMermaidBlockOriginalHTML` / 同等の KaTeX 経路が呼ばれて `reapplyAllMarks` の前提が更新される。描画正しさは引き続き手動視覚チェック + Step PoC（`docs/mdxg-diagram-rendering.archive.md` / `docs/mdxg-math-rendering.archive.md`）に委ねる。`globalThis.__mdxgMermaid` / `__mdxgKatex` は数行の fake で差し替えるため mock コストは許容範囲

  **後回し推奨（テスト brittle 化リスクが構造的）**
  - `src/app/dialog.ts` / `src/app/menu.ts` / `src/app/page-navigation.ts`：event listener cleanup の順序・`stopPropagation` の capture vs bubble・`setTimeout(focus)` などタイマー絡みが多く、fake timer / event 順検証で test 自体が brittle になりやすい。Tier 1〜3 が安定してから着手判断。`page-navigation.ts` の `buildFocusSelector` のような pure helper 部分のみ切り出してテストするのは別途可

  **対象外（IntersectionObserver 未対応）**
  - `src/app/scroll-spy.ts` / `src/app/page-scroll-spy.ts`：observer 起因の挙動（`rootMargin` トリガ・active 切替）は happy-dom 20 で `IntersectionObserver` 未対応のため mock 必須。pure helper の `pickTopmostHeading` は callback ベースで既に node 環境でテスト可能なため、DOM テスト枠で扱う価値は薄い
...
src/review.html (Vite エントリ) ─┼─►  vite + Rolldown   ─►  dist/embed-template.html
                              ...
                              │   + mdxg-shiki-assets      grammar / Mermaid 注入なし最小サイズ、
                              │   + mdxg-split-outputs     ~334 KB / gzip ~101 KB)
vite.config.ts ───────────────┤
                              │                       ─►  dist/standalone.html
                              │                            (単独 Open file 用、
                              │                            Shiki bundled 全言語（約 235） grammar
                              │                            + Mermaid runtime いずれも inline 済み、
                              │                            ~48 MB / gzip ~6.7 MB)
                              │
                              │                       ─►  dist/shiki-langs/<lang>.json
                              │   (mdxg-shiki-assets       (Shiki bundled 全言語（約 235）の grammar JSON、
...
src/mermaid-entry.ts ─────────┐
                              │   vp build --config vite.mermaid.config.ts
                              ├─►  vite + Rolldown   ─►  dist/mermaid.mjs
                              │   (codeSplitting:false で    (1 ファイル ESM、
vite.mermaid.config.ts ───────┘    Mermaid 公式 ESM の        ~3.1 MB / gzip ~859 KB、
                                   dynamic import を本体      bridge `globalThis.__mdxgMermaid`
                                   に inline)                 を末尾で発火、commit 対象、
                                                              CLI / standalone build の入力)

src/katex-entry.ts ───────────┐
                              │   vp build --config vite.katex.config.ts
                              ├─►  vite + Rolldown   ─►  dist/katex/katex.mjs
                              │   (esbuild minify +          (1 ファイル ESM、
vite.katex.config.ts ─────────┘    inlineDynamicImports)     ~259 KB / gzip ~77 KB、
                                                              bridge `globalThis.__mdxgKatex`
                                                              を末尾で発火、commit 対象、
                                                              CLI / standalone build の入力)
                              │
                              │   node scripts/build-katex-css.mjs
                              ├─►  ─►  dist/katex/katex.css
                              │       (minimal: Main / AMS / Math / Size1-4 の
                              │        9 woff2 family を data URI 化 + 全 .katex CSS、
                              │        ~242 KB raw / gzip ~171 KB、commit 対象)
                              │
                              └─►  ─►  dist/katex/katex-fonts-extra.css
                                       (extra: Caligraphic / Fraktur / Script /
                                        SansSerif / Typewriter の 11 family の
                                        @font-face のみ、~128 KB raw / gzip ~95 KB、
                                        CLI `--math-fonts all` / standalone build で
                                        追加注入、commit 対象)
...
vite.review-request.config.ts ─┘                          shebang 付き、
                                                          embed-template.html / mermaid.mjs /
                                                          dist/katex/* を読む)
```
...
3. `dist/katex/katex.mjs` / `dist/katex/katex.css` / `dist/katex/katex-fonts-extra.css` の 3 ファイルが揃っていれば読み込み、`<script id="embedded-katex" type="module">` / `<style id="embedded-katex-css">` / `<style id="embedded-katex-fonts-extra-css">` に inline する（standalone は `--math-fonts all` 相当固定、§12 §14 Math Rendering / `docs/mdxg-math-rendering.archive.md` §5.k）。いずれかが ENOENT のときは stderr に警告を出して skip し、raw `$...$` plain text fallback のみが残る形で書き出す
...
なお `dist/mermaid.mjs` と `dist/katex/*` は `vite.mermaid.config.ts` / `vite.katex.config.ts` / `scripts/build-katex-css.mjs` 側で別途生成されるため、`npm run build` script は `vp build --config vite.mermaid.config.ts && vp build --config vite.katex.config.ts && node scripts/build-katex-css.mjs && vp build && vp build --config vite.review-request.config.ts` の順で実行する（mermaid → katex → standalone inline → CLI bundle の依存順）。
...
| Shiki 関連資材出力          | mdxg-shiki-assets      | buildStart で `src/core/shiki-aliases.generated.ts` を再生成 (Shiki `bundledLanguagesInfo` から 全正規名 + ALIAS_TO_CANONICAL を抽出)、closeBundle で `dist/shiki-langs/<lang>.json` (Shiki bundled 全言語の grammar JSON、現状約 235 個) を emit。共通ロジックは `scripts/lib/shiki-meta.mjs`                                                                                                                                                                                                                                                                                                                                                   |
...
**KaTeX runtime 用（`vite.katex.config.ts` + `scripts/build-katex-css.mjs`）**
...
| レイヤー        | ツール                | 役割                                                                                                                                                                                                                                        |
| --------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| バンドル        | Rolldown（Vite 内蔵） | `src/cli/review-request.ts` を入口に `src/cli/{parse-args,input-source,open-command,serve}.ts` と `src/core/embed.ts` + `src/core/math.ts` を 1 つの ESM (`dist/review-request.mjs`) に統合。Node 組み込みモジュール (`node:*`) は external |
...
```bash
# 1 回ビルド（commit 前に必ず叩く）。mermaid.mjs / katex/* / standalone.html / embed-template.html / review-request.mjs を生成する。
npm run build                  # = vp build --config vite.mermaid.config.ts && vp build --config vite.katex.config.ts && node scripts/build-katex-css.mjs && vp build && vp build --config vite.review-request.config.ts

# Mermaid runtime だけを再ビルド（mermaid 同梱バージョン更新時の差分ビルド用）
npm run build:mermaid          # = vp build --config vite.mermaid.config.ts

# KaTeX runtime + CSS だけを再ビルド（katex 同梱バージョン更新時 / minimal/extra family 分割を見直す時の差分ビルド用）
npm run build:katex            # = vp build --config vite.katex.config.ts && node scripts/build-katex-css.mjs
...
`npm run build` 後に `dist/standalone.html` / `dist/embed-template.html` / `dist/review-request.mjs` / `dist/mermaid.mjs` / `dist/katex/katex.mjs` / `dist/katex/katex.css` / `dist/katex/katex-fonts-extra.css` が再生成される。ソースと生成物の同期は人手 + 任意で pre-commit hook で担保する。`dist/review-request.mjs` は実行時に同ディレクトリの `dist/embed-template.html` を読み込み、`--mermaid` 経路では `dist/mermaid.mjs`、`--math` 経路では `dist/katex/katex.mjs` / `dist/katex/katex.css` / (`--math-fonts all` のときのみ) `dist/katex/katex-fonts-extra.css` も読み込むため、これらすべてが常に揃った状態で commit する。
...
- `app/mermaid.ts`：`isMermaidLike` type guard / `uniqueMermaidId` 連番 / `accumulateUpgradeResult` (ok/failed/skip → UpgradeResult の純粋集約)
- `app/katex.ts`：`isKatexLike` type guard / `isErrorRender` (`katex-error` class 検出による文法エラー判定) / `accumulateUpgradeResult` (ok/failed/skip の純粋集約)
...
- `core/embed.ts`：markdown を JSON 文字列にエンコード（`<` を `<` に置換）、`data-name` 属性エスケープ、`<script id="embedded-md" type="text/markdown">` の rewrite、`<script id="embedded-mermaid" type="module">` への Mermaid runtime 注入 + `</script>` escape 件数返却、`<script id="embedded-katex">` + `<style id="embedded-katex-css">` + (optional) `<style id="embedded-katex-fonts-extra-css">` の 3 ブロックを `rewriteEmbeddedKatex` で同時 rewrite (Mermaid と対称、`</script>` / `</style>` 両方を escape)。Node CLI からもブラウザ側からも再利用できる pure module
- `core/scan-mermaid.ts`：marked.lexer ベースで markdown 内の ` ```mermaid ` フェンス数を数える純粋関数 (CLI `--mermaid auto` の判定中核)
- `core/math.ts`：`scanMath` (text run 内の `$...$` / `$$...$$` を `MathSegment[]` に変換、エスケープ・改行・ネスト境界を含む 22 ケース) / `countMath` (marked.lexer 経由で code / codespan を除外しつつ inline / display 件数集計、CLI `--math auto` の判定中核)
- `cli/parse-args.ts`：CLI 引数パース (`--no-open` / `--document-name` / `--mermaid <auto\|on\|off>` / `--math <auto\|on\|off>` / `--math-fonts <minimal\|all>` / stdin token `-`) と出力ファイル名サニタイズ
...
- `cli/review-request.ts`：CLI エントリの run 関数（引数 → embed → 書き出し → ブラウザ起動の orchestration） + `shouldInjectMermaid` (mode × scanMermaidFences の純粋判定) + `shouldInjectKatex` (mode × countMath の純粋判定、Mermaid と対称)
...
1. ソースは `src/` 配下（`cli/` / `app/` / `core/` / `styles/` / `review.html` / `mermaid-entry.ts` / `katex-entry.ts`）と `scripts/` 配下（`build-katex-css.mjs` 等）のみを編集する
2. ビルド出力（`dist/standalone.html` / `dist/embed-template.html` / `dist/review-request.mjs` / `dist/shiki-langs/*.json` / `dist/mermaid.mjs` / `dist/katex/katex.mjs` / `dist/katex/katex.css` / `dist/katex/katex-fonts-extra.css`）は **手で編集しない**（次の `vp build` / `node scripts/build-katex-css.mjs` で上書きされる）
