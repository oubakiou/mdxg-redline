...
- **祖先 H1 path**: `Page.ancestorHeadingPath` に直近 H1 の ATX 表記 (`# <Title>`) を保持し、`document/doc-renderer.ts` が page スコープで build した blockAnchors の `headingPath` に prepend する。これにより H2 ページ配下のコメントも export 時に H1 祖先を含む完全な headingPath になる (mdxg-virtual-pages.archive.md §9.3)
...
- **URL fragment 形式**: ページ内見出しへの deep link は `#<page-slug>__<heading-slug>` (区切りは `__` 二連 underscore、本実装独自規約、mdxg-virtual-pages.archive.md §6.4)。`document/pages.ts` の `parseHashSlug` / `resolveTargetFromHash` が page slug + heading slug に分解
