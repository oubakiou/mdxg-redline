...
markdown を画面に乗せる経路は 3 つ。起動時の優先順位は §9 を参照。
...
`dist/online.html` がホスティング先 (例: Cloudflare Pages) で配信されているとき、`?url=https://...` クエリで外部 markdown を fetch して描画する経路。standalone.html / embed-template.html ではこの経路は無効化されている（§11.b 配布物の境界）。
...
- **入力 UI**: toolbar の Open URL ボタンから modal で URL を入力可能。submit すると `?url=<encodeURIComponent(input)>` で同一ページが reload される
- **エラー時の挙動**: 7 カテゴリ（`pre_validation_failed` / `redirected_off_allowlist` / `http_error` / `timeout` / `network_error` / `size_exceeded` / `unsupported_content_type`）のいずれかに分類し、persistent な inline empty-state に「別 URL を試す」ボタンと共に表示する
- **fetch 取得 markdown の信頼境界**: 既存の `loadFromMarkdown(text, name)` を経由するため、raw HTML escape / Renderer.link / Renderer.image / Shiki / Mermaid / KaTeX の sanitize 経路はすべて経路 1・2 と同一（§11.a の不変条件）
...
| `--show-open-file`                       | 生成 HTML のヘッダから「Open file」ボタンを残す。既定 hidden は「特定 MD のレビュー固定文脈」前提で別 MD を誤読み込み → `state.comments` 初期化の事故（§3 入力 1）を構造的に防ぐ目的。`standalone.html`（CLI 非経由）は属性が付かず常に表示                                                                                                                                                                       | hidden           |
...
1. オンライン版判定 (<html data-mdxg-online="1"> がビルド時に付与されている場合のみ)
   ...
   1b. URLSearchParams で ?url= を取得
       ...
       - 指定あり + http(s) → fetchMarkdownFromUrl(url, opts, allowlist) を呼ぶ
         (allowlist 検証は wrapper 内で fetch 前 / 後の二段防御、§11.a)
         - 成功: showOnlineSource(finalUrl) + loadFromMarkdown(deriveDocNameFromUrl(url), text)
                 → 2c (embedded-feedback 適用) / 2d (render) に合流
...
3. 該当しなければ空状態のまま `Open file` または `Open URL` (online 版のみ) を待つ
```
...
#### `dist/online.html` の CSP 差分

オンライン版 (§3 入力 3) のみ、`?url=` 経路の外部 fetch と同一オリジン同梱資材 (fingerprinted / canonical 配下の grammar JSON) の runtime fetch を許可するため `connect-src` を `'self'` + allowlist origins に置き換える。他のディレクティブはすべて同一。`'self'` は `docs/feature-online-runtime-assets.md` Phase A.2 で manifest 経由の grammar 動的注入を有効化するために追加 (§5.g)、 allowlist origins は `?url=` 経路の外部 fetch 用 (Phase A 以前から維持)。

| 配布物                     | CSP `connect-src`                                                                               |
| -------------------------- | ----------------------------------------------------------------------------------------------- |
| `dist/standalone.html`     | `'none'`                                                                                        |
| `dist/embed-template.html` | `'none'`                                                                                        |
| `dist/online.html`         | `'self' <allowlist origins>` (既定: `raw.githubusercontent.com` / `gist.githubusercontent.com`) |
...
ホスティング先 (例: Cloudflare Pages) では `_headers` ファイルで HTTP response header としても同じ CSP を返す。meta CSP との single source of truth は `extractCspContent(onlineHtml)` 経由で構造的に担保し、header 行の長さは Cloudflare Pages 上限 2,000 文字を build 時 assertion で守る（`src/build/online-headers.ts`）。
...
ビルドパイプラインは開発者ローカルでのみ動作。配布物 (`dist/standalone.html` / `dist/embed-template.html` / `dist/online.html` / `dist/review-request.mjs` / `dist/shiki-langs/*.json` / `dist/canonical/shiki-langs/*.json` / `dist/fingerprinted/shiki-langs/*.<hash>.json` / `dist/mermaid.mjs` / `dist/katex/katex.mjs` / `dist/katex/katex.css` / `dist/katex/katex-fonts-extra.css` および Cloudflare Pages 用 `dist/_headers` / `dist/_redirects`) はすべてビルド成果物としてリポジトリにコミットされており、エンドユーザー環境にはツールチェーンを持ち込まない。`dist/canonical/` と `dist/fingerprinted/` は online edition の runtime fetch 用 (`docs/feature-online-runtime-assets.md` §5.f / §5.i)、 standalone / CLI 経路では参照しない。

**配布物の信頼境界分離**: `dist/online.html` だけが `'self'` + allowlist 付き `connect-src` を持ち、`dist/standalone.html` / `dist/embed-template.html` は `connect-src 'none'` のままで触らない。3 配布物は共通の `src/review.html` から派生するが、Open URL UI / オンライン版判定 (`data-mdxg-online`) / `<script id="online-allowlist">` / `<script id="online-asset-manifest">` は CSS / JS 両層で `data-mdxg-online="1"` を gate に visible / interactive になる構造で、standalone / embed-template に「壊れた URL UI」や online edition の manifest が混入することを構造的に防ぐ（§13 ビルドパイプライン）。
...
ビルドの出口は 4 つ。エンドユーザーが直接開く配布物 `dist/standalone.html`、review-request CLI が rewrite テンプレートとして読み込む `dist/embed-template.html`、ホスティング配信用の `dist/online.html`（URL fetch viewer、§3 入力 3）、配布者向け CLI ツール `dist/review-request.mjs`。加えてホスティング先 (Cloudflare Pages 等) 用の静的設定ファイル `dist/_headers` / `dist/_redirects` が build 時に emit される。
...
flowchart LR
    subgraph C1["vp build (vite.config.ts)"]
        ...
        B1 --> O1e["dist/online.html"]
        B1 --> O1f["dist/_headers / dist/_redirects"]
        B1 --> O1c["dist/shiki-langs/&lt;lang&gt;.json"]
        B1 --> O1g["dist/canonical/shiki-langs/&lt;lang&gt;.json"]
        B1 --> O1h["dist/fingerprinted/shiki-langs/&lt;lang&gt;.&lt;hash&gt;.json"]
...
| `dist/online.html`                                  | C1   | ホスティング配信用の URL fetch viewer（§3 入力 3）。Phase A.2 で grammar inline を空 `{}` に、`<script id="online-asset-manifest">` を `<head>` に注入。Mermaid / KaTeX は Phase B / C で剥がす予定 (`docs/feature-online-runtime-assets.md` 参照)                                                    | ~4.2 MB / gzip ~1.3 MB |
| `dist/_headers` / `dist/_redirects`                 | C1   | Cloudflare Pages 用静的設定。`_headers` は online.html / `/` に CSP を HTTP response header として返し (`max-age=300`)、`/fingerprinted/*` に `immutable, max-age=31536000`、`/canonical/*` に `max-age=300` の Cache-Control を path 単位で設定。`_redirects` は `/` → `/online.html 200` の rewrite | 数百バイト             |
| `dist/shiki-langs/<lang>.json`                      | C1   | Shiki bundled 全言語の grammar JSON、CLI / standalone の双方が読み込む素材                                                                                                                                                                                                                            | (言語ごとに分割)       |
| `dist/canonical/shiki-langs/<lang>.json`            | C1   | online edition の runtime fetch 用 (hash 無し)。新 deploy 過渡期の 404 retry 先 (`docs/feature-online-runtime-assets.md` §5.i の 3 段 fail-safe)                                                                                                                                                      | (言語ごとに分割)       |
| `dist/fingerprinted/shiki-langs/<lang>.<hash>.json` | C1   | online edition の runtime fetch 用 (content hash 焼き込み済み)。`Cache-Control: immutable` で永久 cache 可。`<script id="online-asset-manifest">` 経由で loader が解決                                                                                                                                | (言語ごとに分割)       |
...
**`mdxg-split-outputs` plugin**: viteSingleFile が中間出力 `dist/review.html` を生成した後、本 plugin の closeBundle が走る。Rollup / Vite の `closeBundle` は parallel hook で plugin 間順序が保証されないため、grammar JSON emit (Shiki bundled 全言語、約 235 個 × 3 系統) も本 plugin の closeBundle 内で `await emitGrammarJsonFiles()` として **逐次** 実行する (`mdxg-shiki-aliases` plugin は `src/core/shiki-aliases.generated.ts` の再生成のみを buildStart で行い、grammar emit は持たない)。役割は次の 7 点：

1. `emitGrammarJsonFiles()` で Shiki bundled 全言語の grammar JSON を生成。生成前に 3 ディレクトリ (`dist/shiki-langs/` / `dist/canonical/shiki-langs/` / `dist/fingerprinted/shiki-langs/`) を `rm({recursive,force})` で clean し、shiki package 更新時の hash 蓄積と SPEC_LANGS 縮小時の孤立ファイルを構造的に排除する。grammar の content hash (SHA-256 先頭 8 桁) を URL に焼き込み、`fingerprinted/shiki-langs/<lang>.<hash>.json` (immutable cache 対象) / `canonical/shiki-langs/<lang>.json` (新 deploy 過渡期の 404 retry 先) / `shiki-langs/<lang>.json` (CLI 配布物用、従来パス) の **3 系統** に同時 emit、戻り値の manifest payload (`{ shikiLangs, mermaid: null, katex: null }`) は本 closeBundle 内のローカル変数として後続処理に渡す
2. `dist/shiki-langs/*.json` を全部読んで grammar の `Record<lang, json>` を組み立て、中間出力 `dist/review.html` の `<script id="embedded-shiki-langs">` に inline
3. `dist/mermaid.mjs` が存在すれば読み込み、`</script>` を `<\/script>` に escape したうえで `<script id="embedded-mermaid" type="module">` に inline（不在時は stderr に警告を出して skip し、Shiki ハイライト fallback のみが残る形で `dist/standalone.html` を書き出す）
4. `dist/katex/katex.mjs` / `dist/katex/katex.css` / `dist/katex/katex-fonts-extra.css` の 3 ファイルが揃っていれば読み込み、`<script id="embedded-katex" type="module">` / `<style id="embedded-katex-css">` / `<style id="embedded-katex-fonts-extra-css">` に inline する（standalone は `--math-fonts all` 相当固定、§12 §14 Math Rendering / `docs/archive/mdxg-math-rendering.archive.md` §5.k）。いずれかが ENOENT のときは stderr に警告を出して skip し、raw `$...$` plain text fallback のみが残る形で書き出す
5. `dist/standalone.html` から `buildOnlineHtml(standaloneHtml, { allowlist, manifest })` で `dist/online.html` を派生。Phase A.2 で確定の 5 mutation を apply: (a) `<html data-mdxg-online="1">` upsert、(b) CSP `connect-src 'none'` → `connect-src 'self' <allowlist origins>` 置換 (`'self'` は同一オリジン同梱資材 fetch 用、`docs/feature-online-runtime-assets.md` §5.g)、(c) `<script id="online-allowlist">` JSON 注入、(d) `<script id="online-asset-manifest">` JSON 注入 (manifest 経由で loader が fingerprinted パスを解決、同 §3.2)、(e) `<script id="embedded-shiki-langs">` の textContent を空 `{}` に上書き (grammar は runtime fetch するため build 時 inline 不要)。allowlist は `buildOnlineAllowlist(process.env)` で env var `MDXG_ONLINE_CONNECT_SRC` を読んで正規化、解決済み allowlist を stdout に emit して build 再現性を保証する。Mermaid / KaTeX runtime は本 phase ではなお inline 維持 (Phase B / C で剥がす予定)
6. `buildOnlineHeadersFile(onlineHtml)` で `dist/_headers` を、`buildOnlineRedirectsFile()` で `dist/_redirects` を emit（Cloudflare Pages 用静的設定。online.html の meta CSP と HTTP header CSP は `extractCspContent(onlineHtml)` 経由で single source of truth、§11.b）。`_headers` の Cache-Control は path 単位で 3 分離: `/` / `/online.html` は `max-age=300`、`/fingerprinted/*` は `immutable, max-age=31536000`、`/canonical/*` は `max-age=300`
7. 中間出力 `dist/review.html` を `dist/embed-template.html` にリネーム（こちらは grammar / Mermaid / KaTeX いずれも注入なし、CLI が `--shiki-langs` / `--mermaid` / `--math` / `--math-fonts` モードに応じて動的に注入する）
...
**grammar の多重生成を許容する根拠**: build 後の `dist/` 配下では grammar JSON が `dist/shiki-langs/<lang>.json` (CLI 配布物)、`dist/canonical/shiki-langs/<lang>.json` (online edition canonical fallback)、`dist/fingerprinted/shiki-langs/<lang>.<hash>.json` (online edition immutable cache 対象)、および `dist/standalone.html` に inline された全 grammar の **4 系統** に存在する。DRY ではないが、すべて同じ `emitGrammarJsonFiles()` の単一呼び出しから生成され、毎 build 冒頭で 3 ディレクトリを clean してから書き出すため drift は構造的に起こらない。standalone は `file://` での fetch path 制約と CSP `connect-src 'none'` を保つため inline 維持、online edition だけが `connect-src 'self'` を許可して runtime fetch する分岐構造（`docs/feature-online-runtime-assets.md`）。symlink 案は Windows / npm publish 互換性が落ちるため不採用。
...
| 出力分岐                    | mdxg-split-outputs     | closeBundle 内で逐次 await: ① `emitGrammarJsonFiles()` で grammar JSON を 3 系統 (CLI / canonical / fingerprinted) に emit + manifest payload を返す。② 中間出力 `dist/review.html` を読み、③ `<script id="embedded-shiki-langs">` に全 grammar を inline + `dist/mermaid.mjs` / `dist/katex/*` が揃っていれば各 embed ブロックに inline (不在時は警告 skip) して `dist/standalone.html` を書き出す。④ standalone から `buildOnlineHtml(standaloneHtml, { allowlist, manifest })` で `dist/online.html` を派生 (Phase A.2 の 5 mutation: `data-mdxg-online`、CSP `'self'` prepend、`<script id="online-allowlist">`、`<script id="online-asset-manifest">`、空 `<script id="embedded-shiki-langs">{}`)。⑤ Cloudflare Pages 用 `dist/_headers` (CSP + path 単位 Cache-Control) / `dist/_redirects` を emit。⑥ 中間出力 `dist/review.html` を `dist/embed-template.html` にリネーム |
...
- **`src/core/`** — 環境非依存の pure module。Node CLI / ブラウザ双方から import される（markdown / block-anchors / page-split / page-outline / slugify / search / embed / escape / feedback / filename-sanitize / review-export / scan-fenced-langs / scan-mermaid / math / footnotes / shiki-aliases.generated / types）
