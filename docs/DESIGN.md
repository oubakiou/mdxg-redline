...
- **DOM 依存ロジックのテスト環境**（採用済み）：`vite.config.ts` の `test.environment` に `happy-dom` を指定し、`src/app/` 配下の DOM API を使うロジック（`mark-engine.ts` の `applyMarksForBlock` / `selection.ts` の `textSegments` / `code-copy-wrap.ts` の `injectCopyButtons` / `doc-renderer.ts` の `cacheBlockOriginalHTML` 等）に in-source test を書けるようにしてある。順序依存（block-id 付与 → wrap 注入 → mark 再適用）のように DOM 階層の更新タイミングが絡む不変条件への regression test を `app/` 配下に貼れる。

  **採用判断の実測値（happy-dom 20.9.0 + vitest 4.1.5）**：
  - baseline (node 環境、DOM テスト 0 件): wall 2.84s / vitest duration 1.31s / `environment 3ms`
  - 採用後 global `happy-dom`（DOM テスト 3 件追加、47 files / 737 tests）: wall **5.05s** / vitest duration 4.55s / `environment 34s` (12 thread 累計)
  - env init コストは **1 ファイルあたり ~200ms × 47 files** が支配的で、`tests` 自体は 280ms。実 DOM が必要なのは 3 件のみだが、global 適用すると残り 734 件の純粋テストも happy-dom 上で走るため env init 累計が課税される。+2.2s wall を「開発中の論理的なランシャビリティを損なわない範囲」と判断して受容
  - 検証して棄却した短縮策：
    - `poolOptions.threads.isolate: false`（worker 内で env 使い回し）→ wall 5.20s で**効果なし**。vp の InlineConfig 型に未定義のため object spread で押し込む必要があり、vitest 本体まで届いていない可能性
    - `fileParallelism: false` + `singleThread: true`（シリアル化で env init を 1 回に集約）→ wall **15.8s** で逆効果。env init 累計 (`environment 10s`) は減るが並列性喪失で wall が大幅悪化
    - per-file 適用：vitest 3+ で `environmentMatchGlobs` が削除済。代替の `test.projects` は vp の `defineConfig` 型が未対応で in-source test との両立可否も未検証
  - **今後の方針**：wall が体感的に重くなった、または env init 累計が更に増えた場合は `test.projects` の vp 型互換性を再評価して per-file 分割を検討する
