...
オンライン版 (§3 入力 3) のみ、`?url=` 経路の fetch を許可するため `connect-src` を allowlist に置き換える。他のディレクティブはすべて同一。

| 配布物                     | CSP `connect-src`                                                                        |
| -------------------------- | ---------------------------------------------------------------------------------------- |
| `dist/standalone.html`     | `'none'`                                                                                 |
| `dist/embed-template.html` | `'none'`                                                                                 |
| `dist/online.html`         | `<allowlist origins>` (既定: `raw.githubusercontent.com` / `gist.githubusercontent.com`) |
...
ビルドパイプラインは開発者ローカルでのみ動作。配布物 (`dist/standalone.html` / `dist/embed-template.html` / `dist/online.html` / `dist/review-request.mjs` / `dist/shiki-langs/*.json` / `dist/mermaid.mjs` / `dist/katex/katex.mjs` / `dist/katex/katex.css` / `dist/katex/katex-fonts-extra.css` および Cloudflare Pages 用 `dist/_headers` / `dist/_redirects`) はすべてビルド成果物としてリポジトリにコミットされており、エンドユーザー環境にはツールチェーンを持ち込まない。

**配布物の信頼境界分離**: `dist/online.html` だけが allowlist 付き `connect-src` を持ち、`dist/standalone.html` / `dist/embed-template.html` は `connect-src 'none'` のままで触らない。3 配布物は共通の `src/review.html` から派生するが、Open URL UI / オンライン版判定 (`data-mdxg-online`) / `<script id="online-allowlist">` は CSS / JS 両層で `data-mdxg-online="1"` を gate に visible / interactive になる構造で、standalone / embed-template に「壊れた URL UI」が混入することを構造的に防ぐ（§13 ビルドパイプライン）。
...
| `dist/online.html`                    | C1   | ホスティング配信用の URL fetch viewer（§3 入力 3）。standalone と同等の内容物 + `<html data-mdxg-online="1">` + CSP `connect-src` allowlist + `<script id="online-allowlist">` JSON | ~48 MB / gzip ~7.0 MB  |
| `dist/_headers` / `dist/_redirects`   | C1   | Cloudflare Pages 用静的設定。`_headers` は online.html / `/` に CSP を HTTP response header として返し、`_redirects` は `/` → `/online.html 200` の rewrite                         | 数百バイト             |
...
**`mdxg-split-outputs` plugin**: viteSingleFile が中間出力 `dist/review.html` を生成した後、`mdxg-shiki-assets` の closeBundle で Shiki bundled 全言語（約 235）の grammar JSON が emit され、続いて本 plugin の closeBundle が走る。役割は次の 6 点：
...
4. `dist/standalone.html` から `buildOnlineHtml(standaloneHtml, { allowlist })` で `dist/online.html` を派生（差分は `<html data-mdxg-online="1">` upsert / CSP `connect-src 'none'` → allowlist origins 置換 / `<script type="application/json" id="online-allowlist">` JSON ブロックの inject の 3 つのみ）。allowlist は `buildOnlineAllowlist(process.env)` で env var `MDXG_ONLINE_CONNECT_SRC` を読んで正規化、解決済み allowlist を stdout に emit して build 再現性を保証する
5. `buildOnlineHeadersFile(onlineHtml)` で `dist/_headers` を、`buildOnlineRedirectsFile()` で `dist/_redirects` を emit（Cloudflare Pages 用静的設定。online.html の meta CSP と HTTP header CSP は `extractCspContent(onlineHtml)` 経由で single source of truth、§11.b）
6. 中間出力 `dist/review.html` を `dist/embed-template.html` にリネーム（こちらは grammar / Mermaid / KaTeX いずれも注入なし、CLI が `--shiki-langs` / `--mermaid` / `--math` / `--math-fonts` モードに応じて動的に注入する）
