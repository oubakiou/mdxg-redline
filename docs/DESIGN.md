...
- `embed-core.ts`：markdown を JSON 文字列にエンコード（`<` を `<` に置換）、`data-name` 属性エスケープ、`<script id="embedded-md" type="text/markdown">` の rewrite。Node CLI からもブラウザ側からも再利用できる pure module
...
│   ├── embed-core.ts        markdown 埋め込みの pure ロジック (JSON エンコード + rewrite + ファイル名規約)。
