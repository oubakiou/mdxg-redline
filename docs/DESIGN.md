...
`dist/standalone.html` を直接開いたときに利用される経路。`Open file` ボタンを押すと OS のファイルダイアログが開き、選んだローカル `*.md` を読み込む（実装は `toolbar.ts` の隠し `<input type="file">` 経由）。
...
| `--show-open-file`                       | 生成 HTML のヘッダから「Open file」ボタンを残す。既定 hidden は「特定 MD のレビュー固定文脈」前提で別 MD を誤読み込み → `state.comments` 初期化の事故（§3 入力 1）を構造的に防ぐ目的。`<html data-toolbar-open-file="off">` を埋め込み、`toolbar.ts` が `#btn-load` / 隠し `<input id="file-md">` を `remove()` する。Comments ▾ メニュー内の Copy / Export / Write feedback.json は常時表示。`standalone.html`（CLI 非経由）は属性が付かず常に表示 | hidden           |
...
整理手段として review-request CLI に `--clean [dir]` サブコマンドを用意してある（`src/cli/clean.ts`）。`<dir>` 直下の `*-<docHash>-review.html` / `*-<docHash>-feedback.json` を命名規約の正規表現 `^(.+)-([0-9a-f]{16})-(review\.html|feedback\.json)$` で抽出し、まとめて削除する。サブディレクトリは触らない（`-maxdepth 1` 相当）。`<dir>` を省略した場合（`--clean` 単独 / `--clean --yes` 等）はカレントディレクトリを対象とする。
...
flowchart LR
    subgraph C1["vp build (vite.config.ts)"]
        direction LR
        I1["src/app/*.ts<br/>src/core/*.ts<br/>src/styles/*.css<br/>src/review.html<br/>vite.config.ts<br/>scripts/lib/shiki-meta.mjs"]
    ...
    subgraph C3["vp build --config vite.katex.config.ts<br/>+ node scripts/build-katex-css.mjs"]
        direction LR
        I3["src/katex-entry.ts<br/>vite.katex.config.ts<br/>scripts/build-katex-css.mjs"]
        B3["vite + Rolldown +<br/>build-katex-css.mjs"]
        I3 --> B3
