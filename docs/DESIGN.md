...
| 判断項目           | 採用                                                                 | 理由                                                                                                                                                                                                             |
| ------------------ | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 言語切替の手段     | ランタイム toolbar toggle + CLI 自体は CLI 出力のみ制御              | CLI と HTML を疎結合に保ち、CLI 生成版と online 版で言語決定ロジックが揃う。レビュー依頼者がレビュワーの初期言語を制御する経路は提供しないが、初回 toggle 後は localStorage で永続化されるため小さなトレードオフ |
...
辞書 2 言語の raw size は約 6–7 KB (~195 entry × 平均 20 bytes × 2 言語)、helper (`i18n-core` / `i18n-browser`) は 1 KB 未満。対応言語数が 3 以上に増えた段階で「全言語を HTML に常時 inline」方式は配布物サイズを線形に押し上げるため、Shiki grammar / Mermaid runtime 等と同じく「必要な言語だけ後から fetch する」方式 (lazy load / 別ファイル分割) への移行を検討する。本実装は 2 言語のみのため inline + 2 state toggle を採用。
