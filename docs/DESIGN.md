...
- **モバイル UI の残課題**：≤768px の主要 chrome（上端固定ヘッダ + footer バー + 左右 drawer + search-bar、navigation 経路の drawer 自動 close、Edit modal trigger の drawer 自動 close、focus 復元契約、breakpoint 切替時の focus 退避）は実装済み（§4 モバイルレイアウト / §10）。次フェーズ候補として以下が残る：
  - **floater (`#floater`) の位置調整 / 選択ハンドル制御**：モバイルのテキスト選択 UX に合わせた floater 位置・選択ハンドルの最適化
  ...
  - **別ページ遷移の再描画コスト（性能 bug、モバイルで顕在化）**：TOC からの別ページ（大セクション）遷移で `navigateToTarget` が `renderAll` → `renderDoc` で `#doc` 全体を再 mount + 全 Shiki / Mermaid / KaTeX を再 upgrade するため、長文 + 低速 CPU で重い。Stacked View は全ページ常駐なのでページ切替で再 mount は本来不要。詳細と修正方針は [docs/bug-stacked-view-pagechange-rerender.md](bug-stacked-view-pagechange-rerender.md)
