#!/usr/bin/env bash
# Cursor postToolUse フック (matcher: Write)。
...
if ! out=$(bash "$(git rev-parse --show-toplevel)/.agents/scripts/check-file.sh" "$file" 2>&1); then
  if [ "$event" = "postToolUse" ]; then
    printf '%s' "$out" | strip_ansi | jq -Rs '{ additional_context: ("check-file failed:\n" + .) }'
  fi
fi
exit 0
