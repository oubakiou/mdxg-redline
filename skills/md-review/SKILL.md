...
```bash
.claude/skills/md-review/scripts/request-review.sh <INPUT_MD> [output-dir]
```
...
```bash
.claude/skills/md-review/scripts/request-review.sh --no-open <INPUT_MD>
```
...
```bash
.claude/skills/md-review/scripts/wait-feedback.sh "$FEEDBACK_JSON"
```
...
.claude/skills/md-review/scripts/request-review.sh path/to/draft.md path/to/output
# stdout に REVIEW_HTML=... と FEEDBACK_JSON=... の 2 行が出る

# レビュワー作業中はここでポーリング (run_in_background 推奨)
.claude/skills/md-review/scripts/wait-feedback.sh /path/to/draft-XXXX-feedback.json
...
```json
{
  "permissions": {
    "allow": [
      "Bash(.claude/skills/md-review/scripts/request-review.sh:*)",
      "Bash(.claude/skills/md-review/scripts/wait-feedback.sh:*)",
